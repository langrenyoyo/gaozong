...
class DouyinLead(Base):
    ...
    raw_data = Column(_JSONStringJSONB(), comment="原始数据JSON")
    ...
    all_extracted_contacts = Column(_JSONStringJSONB(), comment="全部提取到的联系方式 JSON（迁移 0001）")
...
class WechatTask(Base):
    ...
    raw_result = Column(_JSONStringJSONB(), comment="Agent 返回的原始结果 JSON")
...
class DouyinAccountAutoreplySetting(Base):
    ...
    # JSONB 列在 PostgreSQL 为 jsonb，须用 _JSONStringJSONB 跨方言对齐，
    # 否则 psycopg 按 Text 绑定传 VARCHAR 触发 jsonb 隐式转换失败。
    allowed_intents_json = Column(_JSONStringJSONB())
...
class DouyinImageUpload(Base):
    ...
    request_body_json = Column(_JSONStringJSONB(), comment="脱敏后的上游请求 JSON")
    response_body_json = Column(_JSONStringJSONB(), comment="上游响应 JSON 或安全错误详情")
...
class AiAgent(Base):
    ...
    store_address = Column(Text, comment="门店地址")
    store_phone = Column(String(50), comment="门店联系方式")
    store_wechat = Column(String(100), comment="门店微信号")
    business_hours = Column(String(100), comment="门店营业时间")
    sales_cities = Column(Text, comment="销售城市范围")
    sales_brands = Column(Text, comment="销售汽车品牌")
    purchase_cities = Column(Text, comment="收车城市范围")
    purchase_brands = Column(Text, comment="收车汽车品牌")
    after_hours_reply = Column(Text, comment="销售下班时留资回复")
    vehicle_condition_reply = Column(Text, comment="顾客问车况回复")
    appraiser_off_hours_reply = Column(Text, comment="评估师下班时留资回复")
    status = Column(String(20), nullable=False, default="active", comment="active/disabled/deleted")
    created_at = Column(DateTime, default=datetime.now)
    updated_at = Column(DateTime, default=datetime.now, onupdate=datetime.now)
...
class AgentKnowledgeCategory(Base):
    ...
    is_base = Column(_IntegerBoolean, nullable=False, default=0, comment="是否 base 分类，0/1")
