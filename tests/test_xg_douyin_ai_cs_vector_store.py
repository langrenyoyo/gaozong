...
def test_sqlite_backend_does_not_require_milvus_config_or_dependency(monkeypatch):
    _clear_milvus_env(monkeypatch)
...
def test_milvus_backend_missing_required_config_is_rejected(monkeypatch):
    ...
    monkeypatch.setenv("MILVUS_TOKEN", "secret-token-should-not-leak")
    ...
    assert "secret-token-should-not-leak" not in str(exc_info.value)


def test_milvus_backend_missing_dependency_is_rejected_without_token_leak(monkeypatch):
    _clear_milvus_env(monkeypatch)
    monkeypatch.setenv("RAG_VECTOR_BACKEND", "milvus")
