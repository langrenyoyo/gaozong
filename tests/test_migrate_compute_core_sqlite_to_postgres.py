...
from scripts import smoke_migrate_compute_core_dev_apply as smoke
...
def _snapshot(*, keys=None):
    return migration.TargetSnapshot(
        alembic_revision="0005_compute_core",
...
def test_table_order_and_table_filter_are_deterministic():
    assert migration.DEFAULT_TABLES == ["compute_accounts", "compute_transactions"]
    assert migration.parse_tables("compute_transactions") == ["compute_transactions"]
    with pytest.raises(migration.MigrationConfigurationError, match="不支持的表"):
        migration.parse_tables("compute_packages")


def test_field_mapping_reports_ignored_and_defaulted_fields():
    mapped = migration.map_source_row(
        "compute_accounts",
        {"id": 1, "merchant_id": "m1", "legacy_only": "x"},
    )

    assert mapped.row["balance_tokens"] == 0
    assert "legacy_only" in mapped.ignored_fields
    assert "balance_tokens" in mapped.defaulted_fields


def test_datetime_parse_failure_becomes_error_row():
    summary = migration.build_table_plan(
        "compute_accounts",
        [{"id": 1, "merchant_id": "m1", "created_at": "bad-time"}],
        _snapshot(),
    )
...
def test_token_fields_remain_int_and_non_integer_values_are_errors():
    mapped = migration.map_source_row(
        "compute_transactions",
        {
            ...
            "delta_tokens": "100",
            "balance_after_tokens": "100",
            "source": "manual_recharge",
        },
    )
    assert isinstance(mapped.row["delta_tokens"], int)
    assert isinstance(mapped.row["balance_after_tokens"], int)
    ...
    assert summary.error_rows == 1
    assert "delta_tokens" in summary.errors[0].reason


def test_delta_tokens_zero_is_rejected_before_apply():
    summary = migration.build_table_plan(
        "compute_transactions",
        [
            {
    ...
    assert summary.error_rows == 1
    assert "delta_tokens 不能为 0" in summary.errors[0].reason


def test_sensitive_context_is_masked_in_mapping_preview():
    summary = migration.build_table_plan(
        "compute_transactions",
        [
            {
    ...
    assert "客户完整对话上下文" not in preview
    assert "agent-secret" not in preview
    assert "987654321" not in preview
...
def test_missing_table_has_clear_error(tmp_path):
    ...
    with pytest.raises(migration.MigrationReadError, match="SQLite 表不存在: compute_accounts"):
        migration.read_sqlite_tables(str(db_path), ["compute_accounts"])


def test_upsert_keys_match_p3f1_constraints():
    assert migration.TABLE_CONFIGS["compute_accounts"].upsert_key == ("merchant_id",)
    assert migration.TABLE_CONFIGS["compute_transactions"].upsert_key == ("id",)
...
def test_no_delete_truncate_drop_sql_is_generated():
    ...
    assert "insert into compute_accounts" in sql_text
...
def test_estimates_insert_update_skip_from_snapshot():
    ...
    summary = migration.build_table_plan(
        "compute_accounts",
        [
            {"id": 1, "merchant_id": "m1", "balance_tokens": 100},
            {"id": 2, "merchant_id": "m2", "balance_tokens": 200},
            {"id": 3, "merchant_id": "m2", "balance_tokens": 300},
        ],
        snapshot,
    )
...
def test_tables_option_only_migrates_selected_table():
    ...
    assert list(plan.tables) == ["compute_transactions"]
...
def test_smoke_fixture_contains_only_synthetic_compute_rows(tmp_path):
    sqlite_path = tmp_path / "compute_smoke.db"
    smoke.create_fixture_sqlite(sqlite_path)

    rows = migration.read_sqlite_tables(str(sqlite_path), migration.DEFAULT_TABLES)
    assert len(rows["compute_accounts"]) == 2
    assert len(rows["compute_transactions"]) == 2
    assert {row["merchant_id"] for row in rows["compute_accounts"]} == set(smoke.SYNTHETIC_MERCHANT_IDS)
    assert {row["delta_tokens"] for row in rows["compute_transactions"]} == {200, -50}
    assert all(row["delta_tokens"] != 0 for row in rows["compute_transactions"])


def test_smoke_requires_smoke_database_url(monkeypatch):
    monkeypatch.delenv(smoke.SMOKE_ENV_NAME, raising=False)

    with pytest.raises(smoke.SmokeError, match="缺少 SMOKE_DATABASE_URL"):
