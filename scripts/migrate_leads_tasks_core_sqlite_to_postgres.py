"""四张线索与任务核心表 SQLite -> PostgreSQL 迁移脚本。

默认 dry-run。P3-D2 只允许本地/dev PostgreSQL 受控 apply smoke，不碰宝塔生产。
"""
...
DEFAULT_TABLES = ["sales_staff", "douyin_leads", "douyin_webhook_events", "wechat_tasks"]
...
TABLE_CONFIGS: dict[str, TableConfig] = {
    ...
    "douyin_webhook_events": TableConfig(
        columns=(
            "id",
            "tenant_id",
            "merchant_id",
        ...
        upsert_key=("event_key",),
        json_fields=("parsed_content_json", "raw_body"),
        datetime_fields=("message_create_time", "created_at"),
        bool_fields=("is_duplicate",),
        int_fields=("id", "lead_id"),
        defaults={"is_duplicate": False, "raw_body": {}},
        required_fields=("id",),
        update_columns=(
            "tenant_id",
            "merchant_id",
    ...
    "douyin_leads": TableConfig(
        ...
        upsert_key=("account_open_id", "conversation_short_id"),
        json_fields=("raw_data", "all_extracted_contacts"),
        datetime_fields=("assigned_at", "created_at", "updated_at"),
        int_fields=("id", "assigned_staff_id", "reassign_count"),
        defaults={"source": "douyin", "status": "pending", "reassign_count": 0},
        required_fields=("id",),
        update_columns=(
            "source",
            "lead_type",
    ...
    "wechat_tasks": TableConfig(
        ...
        upsert_key=("id",),
        json_fields=("raw_result",),
        datetime_fields=("pasted_at", "sent_at", "created_at", "updated_at"),
        int_fields=("id", "lead_id", "staff_id", "reply_check_id", "agent_pid"),
        defaults={"task_type": "notify_sales", "mode": "paste_only", "status": "pending"},
        required_fields=("id",),
        update_columns=(
            "tenant_id",
            "merchant_id",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="dry-run/apply 迁移 4 张 leads/tasks core 表。")
...
def map_source_row(table: str, source_row: Mapping[str, object]) -> MappedRow:
    ...
    if table == "douyin_webhook_events" and not row.get("event_key"):
        row["event_key"] = f"legacy:{row.get('id')}"
        defaulted_fields.append("event_key")
        warnings.append("event_key 缺失，使用 legacy:id 作为 dry-run/dev smoke 幂等键")
...
def mask_preview_row(row: Mapping[str, object]) -> dict[str, object]:
    masked = dict(row)
    for field in ("customer_contact", "extracted_phone", "phone"):
        if masked.get(field):
            masked[field] = mask_contact(str(masked[field]))
    if masked.get("extracted_wechat"):
        masked["extracted_wechat"] = "***"
    return masked
...
def build_upsert_sql(table: str) -> str:
    ...
    conflict_key = ", ".join(_quote_column(column) for column in config.upsert_key)
    ...
    if not update_clause:
        update_clause = f"{_quote_column(columns[-1])} = EXCLUDED.{_quote_column(columns[-1])}"
    return (
...
def _quote_column(column: str) -> str:
    return f'"{column}"' if column in {"event"} else column
...
async def apply_postgres_rows(
    ...
    try:
        async with conn.transaction():
            for table in tables:
                summary = build_table_plan(table, source_rows_by_table.get(table, []), snapshot)
                if summary.error_rows:
                    results[table] = ApplyTableResult(
                        ...
                        after_count=snapshot.existing_counts.get(table, 0),
                    )
                    continue
                seen_keys: set[tuple[object, ...]] = set()
                inserted = 0
                ...
                for source_row in source_rows_by_table.get(table, []):
                    ...
                    if key in seen_keys:
                        ...
                        continue
                    seen_keys.add(key)
                    was_inserted = await conn.fetchval(build_upsert_sql(table), *row_to_params(table, mapped.row))
                    if was_inserted:
                        inserted += 1
                    else:
                        updated += 1
