"""9000 SQLite -> PostgreSQL cutover 一次性迁移脚本。

默认 dry-run。apply 只允许 dev/staging 显式确认，不允许隐式使用 DATABASE_URL。
"""
...
class ColumnMapping:
    ...
    synthetic_columns: dict[str, str]
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="dry-run/apply 迁移 9000 runtime cutover 表。")
    ...
    parser.add_argument("--tables", default=",".join(CUTOVER_TABLES), help="逗号分隔表名，默认 30 张表。")
...
def build_column_mapping(table: str, sqlite_columns: set[str], pg_columns: set[str]) -> ColumnMapping:
    synthetic_columns: dict[str, str] = {}
    if table == "knowledge_categories" and "key" in pg_columns and "key" not in sqlite_columns and "category_key" in sqlite_columns:
        synthetic_columns["key"] = "category_key"

    shared_columns = sorted((sqlite_columns & pg_columns) - set(synthetic_columns))
    copy_columns = (["id"] if "id" in shared_columns else []) + [column for column in shared_columns if column != "id"]
    ignored = sorted(sqlite_columns - pg_columns)
    defaulted = sorted(pg_columns - sqlite_columns - set(synthetic_columns))
...
def build_table_plan(
    ...
    existing_ids = snapshot.existing_ids.get(table, set())
    ...
    for source_row in rows:
        try:
            ...
            row_id = mapped.get("id")
            if row_id is None:
                raise ValueError("id 缺失")
            row_id_int = int(row_id)
            if row_id_int in seen_ids:
                skip_count += 1
                continue
            seen_ids.add(row_id_int)
...
def map_row(source_row: Mapping[str, object], mapping: ColumnMapping) -> tuple[dict[str, object], list[str]]:
    ...
    for column in mapping.copy_columns:
        ...
        result[column] = value
    for target_column, source_column in mapping.synthetic_columns.items():
        result[target_column] = source_row.get(source_column)
...
def mask_preview_row(row: Mapping[str, object]) -> dict[str, object]:
    ...
    for field, value in list(masked.items()):
        ...
        if "phone" in name or "contact" in name:
            masked[field] = mask_contact(str(value))
...
async def read_postgres_snapshot(database_url: str, tables: Sequence[str]) -> TargetSnapshot:
    ...
    try:
        ...
        for table in tables:
            ...
            if "id" in columns[table]:
                existing_ids[table] = {int(row["id"]) for row in await conn.fetch(f'SELECT id FROM "{table}"')}
        return TargetSnapshot(revision, table_exists, columns, existing_ids, counts)
    finally:
        await conn.close()


def build_upsert_sql(table: str, columns: Sequence[str]) -> str:
    insert_columns = [column for column in columns if column != "pg_only"]
    quoted_columns = ", ".join(quote_ident(column) for column in insert_columns)
    placeholders = ", ".join(f"${index}" for index, _ in enumerate(insert_columns, start=1))
    update_columns = [column for column in insert_columns if column != "id"]
    update_clause = ", ".join(f"{quote_ident(column)} = EXCLUDED.{quote_ident(column)}" for column in update_columns) or "id = EXCLUDED.id"
    return (
    ...
    try:
        async with conn.transaction():
            for table, table_plan in plan.tables.items():
                ...
                columns = mapping.copy_columns + list(mapping.synthetic_columns)
