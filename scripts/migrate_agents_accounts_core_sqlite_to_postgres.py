"""agents/accounts 四表 SQLite -> PostgreSQL 迁移脚本。

默认 dry-run。P3-E2 只允许本地/dev PostgreSQL 受控 apply smoke，不碰宝塔生产。
"""
...
TABLE_CONFIGS: dict[str, TableConfig] = {
    "ai_agents": TableConfig(
        ...
        int_fields=("id",),
        defaults={"avatar_seed": "default", "prompt": "", "knowledge_base_text": "", "status": "active"},
        required_fields=("agent_id", "merchant_id", "name", "avatar_seed"),
        update_columns=(
            "merchant_id",
            "name",
            "avatar_seed",
    ...
    "douyin_authorized_accounts": TableConfig(
        columns=(
            "id",
            "merchant_id",
            "tenant_id",
        ...
        upsert_key=("merchant_id", "open_id"),
        json_fields=("raw_body_json",),
        datetime_fields=(
            "bind_time",
            "unbind_time",
            "source_created_at",
        ...
        int_fields=("id", "main_account_id", "bind_status", "account_type"),
        defaults={"bind_status": 0},
        required_fields=("main_account_id", "open_id"),
        update_columns=(
            "tenant_id",
            "main_account_id",
            "user_id",
    ...
    "douyin_account_agent_bindings": TableConfig(
        columns=(
            "id",
            "merchant_id",
            "tenant_id",
        ...
        upsert_key=("id",),
        datetime_fields=("created_at", "updated_at", "unbound_at", "deleted_at"),
        bool_fields=("is_default",),
        int_fields=("id", "douyin_authorized_account_id"),
        defaults={"is_default": True, "status": "active"},
        required_fields=("id", "merchant_id", "account_open_id", "agent_id"),
        update_columns=(
            "merchant_id",
            "tenant_id",
            "account_open_id",
    ...
    "agent_knowledge_categories": TableConfig(
        columns=(
            "id",
            "merchant_id",
            "tenant_id",
        ...
        upsert_key=("merchant_id", "agent_id", "category_key"),
        datetime_fields=("created_at", "updated_at", "deleted_at"),
        bool_fields=("is_base",),
        int_fields=("id",),
        defaults={"scope_type": "merchant", "is_base": False, "status": "active"},
        required_fields=("merchant_id", "agent_id", "category_key"),
        update_columns=(
            "tenant_id",
            "scope_type",
            "is_base",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="dry-run/apply 迁移 agents/accounts 四表。")
    ...
    parser.add_argument("--tables", default=",".join(DEFAULT_TABLES), help="逗号分隔表名，默认四表全部。")
...
def map_source_row(table: str, source_row: Mapping[str, object]) -> MappedRow:
    ...
    for column in config.columns:
        ...
        if raw_value is None and column in defaults:
            ...
            defaulted_fields.append(column)
        value = raw_value
        if column in config.json_fields:
            value, warning = _coerce_json(column, raw_value)
            if warning:
                warnings.append(warning)
        elif column in config.datetime_fields:
            value = _coerce_datetime(column, raw_value)
        elif column in config.bool_fields:
            value = _coerce_bool(raw_value)
        elif column in config.int_fields:
            value = _coerce_int(column, raw_value)
        row[column] = value
    if table == "douyin_authorized_accounts" and not row.get("merchant_id"):
        warnings.append("merchant_id 缺失：保留原值并按历史数据保守迁移，后续接口层仍应拒绝无商户归属账号")
...
def build_table_plan(table: str, rows: Sequence[Mapping[str, object]], snapshot: TargetSnapshot) -> TablePlan:
    ...
    for source_row in rows:
        try:
            ...
            semantic_error = _detect_semantic_conflict(table, mapped.row, semantic_keys)
            if semantic_error:
...
def mask_preview_row(row: Mapping[str, object]) -> dict[str, object]:
    ...
    for field, value in list(masked.items()):
        ...
        if value is None:
            continue
        if "token" in name or "secret" in name or name.endswith("open_id") or name in {"user_id", "union_id"}:
            masked[field] = mask_identifier(str(value))
        elif field == "raw_body_json":
            masked[field] = "<redacted_json>"
    return masked
...
def _coerce_json(column: str, value: object) -> tuple[object, str | None]:
...
def _coerce_datetime(column: str, value: object) -> datetime | date | None:
    ...
    if text.isdigit():
        return _coerce_datetime(column, int(text))
    try:
...
def _coerce_int(column: str, value: object) -> int | None:
    ...
    try:
        ...
        raise ValueError(f"{column} int 解析失败: {value}") from exc
...
def _detect_semantic_conflict(
    ...
    semantic_keys: set[tuple[object, ...]],
) -> str | None:
    if table == "douyin_account_agent_bindings" and _is_active_default_binding(row):
        key = (row.get("merchant_id"), row.get("account_open_id"))
        if key in semantic_keys:
            return f"active default binding 冲突: merchant_id={key[0]} account_open_id={mask_identifier(str(key[1]))}"
        semantic_keys.add(key)
    if table == "agent_knowledge_categories" and _is_active_knowledge_category(row):
        key = (row.get("merchant_id"), row.get("agent_id"), row.get("category_key"))
        if key in semantic_keys:
            return f"active knowledge category 冲突: merchant_id={key[0]} agent_id={key[1]} category_key={key[2]}"
        semantic_keys.add(key)
    return None


def _is_active_default_binding(row: Mapping[str, object]) -> bool:
    return bool(row.get("is_default")) and row.get("status") == "active" and row.get("deleted_at") is None


def _is_active_knowledge_category(row: Mapping[str, object]) -> bool:
    return row.get("status") == "active" and row.get("deleted_at") is None
...
def build_upsert_sql(table: str, row: Mapping[str, object] | None = None) -> str:
    ...
    conflict_key, conflict_where = _conflict_target(table, row)
    conflict_columns = ", ".join(_quote_column(column) for column in conflict_key)
    update_columns = [column for column in config.update_columns if column not in conflict_key and column != "id"]
    update_clause = ", ".join(f"{_quote_column(column)} = EXCLUDED.{_quote_column(column)}" for column in update_columns)
    conflict_clause = f"ON CONFLICT ({conflict_columns})"
    if conflict_where:
        conflict_clause += f" WHERE {conflict_where}"
    return (
        f"INSERT INTO {table} ({quoted_columns}) VALUES ({placeholders}) "
        f"{conflict_clause} DO UPDATE SET {update_clause} "
        "RETURNING (xmax = 0) AS inserted"
    )


def _conflict_target(table: str, row: Mapping[str, object] | None) -> tuple[tuple[str, ...], str | None]:
    if table == "agent_knowledge_categories" and (row is None or _is_active_knowledge_category(row)):
        return TABLE_CONFIGS[table].upsert_key, "status = 'active' AND deleted_at IS NULL"
    return TABLE_CONFIGS[table].upsert_key, None
...
def row_to_params(table: str, row: Mapping[str, object]) -> tuple[object, ...]:
    ...
    for column in config.columns:
        ...
        if column in config.json_fields and isinstance(value, (dict, list)):
