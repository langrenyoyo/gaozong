...
| `ai_edit_materials` | 商户私有素材 metadata、本地/云端状态与内部 7 天清理；历史平台字段只作兼容保留 |
...
- [x] **Step 2: 19000 本地素材与进程监管**
  - 已落地本地受管素材、元数据同步、活动引用删除守卫和 7 天软删除时间戳；跨端自动到期清理尚未完成，明确留 Task 12-7，产品不提供回收站。
...
- [x] **Step 5: 前端基础闭环**
  - 已实现基础素材库、任务进度、720P/1080P 任务操作和本地导入；真实素材预览、自动分析和商户私有云端上传明确留 Task 12。
