...
| 3 | `apps/xg_douyin_ai_cs/services/reply_decision_service.py:3800` | M01 | Auto Reply | ACTIVE（:1160/:1236 primary+retry_combined） | `f"ai_auto_reply_run:{run_id}:{attempt_count}:{llm_call_stage}"` | 否（9000 `ai_auto_reply_dry_run_service.py:382-383` 在 `_add_run` 提交后注入 run.id+attempt_count，commit-before-9100-call；aa58f96 已独立验收） | ACTIVE |
