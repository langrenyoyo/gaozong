...
Service: apps/agents/services.py:98-122 update_agent
  ...
  → 含 _STORE_CONFIG_FIELDS 11 个 V2.0 门店字段循环 setattr
Table: ai_agents
```
...
Router: agents.py:194-315 preview_agent
  ...
  → :208-212 若带 agent_id → get_agent(merchant 过滤) 校验归属
  → :216-219 ensure_category_usable_for_merchant
  → :241-262 组装 agent_config dict（来源：前端 payload 草稿值，非 DB）
    字段：system_prompt/prompt/knowledge_base_text/status/allowed_category_keys/
          rag_enabled + 11 商家变量
  → :267-268 补 direct_llm_policy + forbidden_words
  → :272-276 get_xg_douyin_ai_cs_client().suggest_reply(
...
webhook 事件 → run_ai_auto_reply_job(event_id)
  ...
  → :315-336 组装 agent_config（来源：binding.agent DB 模型，非前端 payload）
    字段结构与 preview 完全一致（system_prompt/prompt/knowledge_base_text/11 变量）
