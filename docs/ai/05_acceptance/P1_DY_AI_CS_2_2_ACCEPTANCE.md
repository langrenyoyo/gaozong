...
1. 2.2.2 抖音企业号列表已基本满足头像、昵称、未读消息数展示；`unread_count` 已读/未读协议已通过独立测试（候选 `8e69adc`，T1 A1-A14 PASS，241+10+10=261 passed），使用 `last_seen_event_id` + `(created_at, event_id)` 单调水位计算。
...
10. 下一步建议优先确认在线状态来源，再评审媒体能力与自动发送安全方案；已读状态协议已通过独立测试（候选 `8e69adc`，T1 PASS，261 passed），使用 `last_seen_event_id` + `(created_at, event_id)` 单调水位。
...
| 2.2.3 | 未读消息数 | 已通过独立测试 | `list_account_conversations()` 中 `unread_count` 按 `(created_at, event_id)` 水位统计 `im_receive_msg`；前端会话 item 展示 | 通过 | 与企业号未读一致，使用 `last_seen_event_id` 水位（候选 `8e69adc`，T1 PASS）。 |
...
7. 未读数已读/未读协议已通过独立测试（候选 `8e69adc`，T1 A1-A14 PASS，261 passed）：`mark-read` 请求必填 `last_seen_event_id`，服务端验证事件归属后精确推进 `(created_at, event_id)` 单调水位，前端仅在详情成功渲染后提交；非私信事件和空 `created_at` 事件拒绝推进；并发通过 DB 条件更新和 `IntegrityError` 恢复保护。
8. 当前在线状态没有真实来源时显示 `unknown` / 状态未知，不伪造在线状态。
9. 本轮验收没有调用真实抖音发送、真实 LLM、真实 Embedding、微信自动化、真实支付或数据库迁移。

## 6. 剩余缺口

1. 企业号与会话 `unread_count` 已读/未读协议已通过独立测试（DY-CS-CONVERSATION-READ-PROTOCOL-1 候选 `8e69adc`，T1 A1-A14 PASS，261 passed），使用 `last_seen_event_id` + `(created_at, event_id)` 单调水位；候选尚未推送、合并或发布，未验证真实 PostgreSQL 和生产环境。
...
| `DY-CS-CONVERSATION-READ-PROTOCOL-1` 独立测试 | 候选 `8e69adc`，Test-Revision T1，A1-A14 全部验收通过，任务级结论 PASS | 指定专项及回归集 241 passed, 0 failed；IntegrityError 竞争重复 10 次通过；Barrier 双线程竞争重复 10 次通过；合计 261 passed, 0 failed；Python 语法检查通过；前端 TypeScript 检查通过。 |
...
| `P1-DYCS-READSTATE-1` | 真实已读/未读状态设计 | 已通过独立测试（候选 `8e69adc`，T1 A1-A14 PASS，261 passed），使用 `last_seen_event_id` + `(created_at, event_id)` 单调水位（DY-CS-CONVERSATION-READ-PROTOCOL-1） | `mark_conversation_read` 服务端、前端渲染后提交、测试 | 高 | 候选尚未推送、合并或发布；未验证真实 PostgreSQL 和生产环境。 |
