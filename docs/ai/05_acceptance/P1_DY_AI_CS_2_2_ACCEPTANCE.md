...
1. 2.2.2 抖音企业号列表已基本满足头像、昵称、未读消息数展示；其中 `unread_count` 当前是一阶段临时口径，统计入站 `im_receive_msg` 数量，不是真正已读/未读系统。
...
10. 下一步建议优先确认在线状态来源和真实已读状态，再评审媒体能力与自动发送安全方案。
...
| 2.2.2 | 未读消息数 | 安全降级实现 | `app/services/douyin_workbench_conversation_service.py:get_account_unread_counts()`；`app/routers/douyin_accounts.py` 返回 `unread_count`；`tests/test_douyin_accounts_router.py` 未读测试 | 通过 | 临时统计入站 `im_receive_msg`，后续需真实 read state。 |
...
| 2.2.3 | 未读消息数 | 安全降级实现 | `list_account_conversations()` 中 `unread_count` 统计 `im_receive_msg`；前端会话 item 展示 | 通过 | 与企业号未读同属临时口径。 |
...
7. 当前未读数是一阶段临时口径，不是真正 read/unread 系统。
8. 当前在线状态没有真实来源时显示 `unknown` / 状态未知，不伪造在线状态。
9. 本轮验收没有调用真实抖音发送、真实 LLM、真实 Embedding、微信自动化、真实支付或数据库迁移。

## 6. 剩余缺口

1. 企业号与会话 `unread_count` 仍是一阶段临时口径，未来需要 `read_at` / `last_read_message_id` 或抖音侧已读事件。
...
| `P1-DYCS-READSTATE-1` | 真实已读/未读状态设计 | 从临时入站计数升级为真实 read/unread 体系 | 数据模型方案、迁移方案、9000 聚合服务、测试 | 高 | 先输出设计与迁移方案，再补测试；不得直接改迁移。 |
