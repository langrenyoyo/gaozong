...
| P1-F | `/list_bind_info` 授权账号同步和持久化 | 已落库 `douyin_authorized_accounts`，账号列表 HTTP 路由始终要求可信商户上下文并只返回当前商户有效持久化绑定账号（无有效绑定的 webhook 事件账号不可见）；内部服务 `merchant_id=None` 兼容分支保留 live-check/event fallback，但当前 HTTP 路由不会进入 |
