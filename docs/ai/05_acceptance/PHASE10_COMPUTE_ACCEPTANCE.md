...
## 9. COMPUTE-OPT-03 管理员算力配置接入（已完成独立测试并推送，生产验证待后续窗口）
...
| `fa1d0c0` | 提交 C | 文档原位更新（本节 + 05_PROJECT_CONTEXT.md + 总设计阶段三状态） |
| `ea6a5d9` | 提交 D | 收敛管理员算力配置页面用语（SuperComputeConfig.tsx 计费比例说明文案） |
| `417d68c` | 最终合并提交 | 双父合并（parent1=`dc3d862` master 一侧 / parent2=`2b6f554` Final-Tested-Source），解决 `frontend/src/pages/Index.tsx` 冲突（保留主线微信助手分支，紧随 `admin-forbidden-words` 加入 `admin-compute-config` 入口，删除旧 `isComputeConfigNav` 变量与旧商户渲染分支）；已通过独立测试并推送 master |
...
- **后端 compute 冻结集（11 文件）**：`180 passed, 0 failed`，退出码 0。COMPUTE-OPT-03 R1（提交 `7ac4a7e`）已修正 `test_9000_update_affects_new_usage_not_old_snapshot`：不再从商户公开接口 `/compute/transactions` 读取内部字段（COMPUTE-OPT-02 已脱敏 `markup_basis_points`/`actual_tokens`），改为通过 `TestSession` 直接查询 `ComputeTransaction`（`merchant_id`/`transaction_type=consume`/按 `id` 倒序/`try-finally` 关闭），原快照验证语义不变。
- **前端导航测试** `tests/test_frontend_capability_navigation.py`：`11 passed, 2 failed`，退出码 1。两个 failed 为 pre-existing 基线问题（`capabilities.ts` title 断言、`features/agents/components` 目录缺失），在基线 `dc3d862` 上对照运行结果与候选完全一致，非本包引入。
- **前端合同/构建/类型**：`npm run phase10-compute:check` 退出码 0；`npm run build` 退出码 0（chunk size 警告 pre-existing）；`npx tsc --noEmit -p tsconfig.app.json` 退出码 0。
- **定向 eslint**：候选 51 problems（42 errors、9 warnings），基线 56 problems（47 errors、9 warnings），两者退出码均为 1，相对基线零新增（本包将 SuperComputeConfig 两处 effect 合并为一处，错误数减少）。剩余 errors 为 `react-hooks/set-state-in-effect` 基线前端技术债。
- **Edge headless + CDP 验收**：独立测试窗口通过 Edge 无头模式 + Chrome DevTools Protocol 纯文本 JSON 验收，覆盖两视口（1024/1440）、三视图（ratios/packages/merchant-grant）、旧地址重定向（`/compute/packages`、`/compute/markup-ratios`）、页面状态、四类成功操作和权限拒绝分支。

### 9.4 残余风险

- `baota_production_compute_not_verified`：未变，宝塔生产算力链路仍待后续生产验证窗口。
- 两个基线一致的前端导航测试失败：`test_frontend_capability_navigation_has_only_product_centers...`（`capabilities.ts` title 断言）与 `test_frontend_feature_directories_have_required_entrypoints`（`features/agents/components` 目录缺失），候选与基线 `dc3d862` 失败节点完全一致，pre-existing。
- eslint 基线技术债：`react-hooks/set-state-in-effect` 残留错误，属基线前端技术债，修复需重构数据获取架构（超出本包边界）。
- 未执行宝塔生产真实发送/支付验证。

### 9.5 真实外部调用 = 0

执行窗口未启动服务、未连宝塔/生产 PG、未触发真实付费模型调用。Edge headless + CDP 验收为本地无头浏览器纯文本 JSON 证据，未连接生产环境。
