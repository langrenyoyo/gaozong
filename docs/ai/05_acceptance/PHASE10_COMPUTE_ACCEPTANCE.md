...
- **后端 compute 冻结集（11 文件）**：`179 passed, 1 failed`，1 个已由基线 `e2d439e` 对照确认的既有失败。唯一 failed 为 `test_phase10_compute_markup_api.py::test_9000_update_affects_new_usage_not_old_snapshot`（`KeyError: 'markup_basis_points'`），属 COMPUTE-OPT-02 将 `/compute/transactions` 收敛为 7 字段公开投影的既有残留，基线 stash 对照零新增，超出 COMPUTE-OPT-03 边界不可触碰。
