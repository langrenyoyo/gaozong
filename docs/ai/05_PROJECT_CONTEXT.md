...
- **2026-08-09 项目治理阶段 3A P1 COMPUTE-IDEMPOTENCY-001 实施进行中（6/10 charge path MIGRATED）**：按 CROSS_MODULE_RISK_REGISTER P1 优先级，先做技术方案设计（`docs/architecture/remediation/P1_COMPUTE_IDEMPOTENCY_TECHNICAL_DESIGN.md`，TECHNICAL_DESIGN_APPROVED）+ M01 身份验真（`docs/architecture/remediation/P1_M01_IDENTITY_VERIFICATION.md`，结论 B：run.id + attempt_count + llm_call_stage 三维 identity），再分 Stage 实施。**M07 Core**（commit cb86c3b/2e81eaf/4109f9d）：record_usage 加 idempotency_key + payload_evidence（stable inputs SHA-256：capability_key/model/tokens/usage_measurement_method/llm_call_stage，EXCLUDE source/agent_id/conversation_id）+ DB migration 0030（compute_transactions 加 idempotency_key + payload_evidence + UniqueConstraint(merchant_id, idempotency_key)）+ 事务原子性修正（INSERT flush → balance UPDATE → 单次 commit，非分次 commit）+ IntegrityError replay/conflict handling（非 ON CONFLICT）+ None 兼容路径（warning 可追踪）+ PG_CORE_RELEASE_GATE PASS（PostgreSQL 并发：同 idempotency identity → 1 txn / 1 balance mutation / 不同 payload → IDEMPOTENCY_CONFLICT / winner evidence 不被覆盖）。**Consumer 迁移**（6/10 MIGRATED）：① M04 `wechat_task:{task.id}:result_usage`（E2E_VERIFIED_FIXED_FOR_M04，原失败基线复跑证明 double charge 修复）② M06 `las_job:{job.id}:archive_usage`（IDEMPOTENCY_CONTRACT_MIGRATED_AND_VERIFIED）③ M01 Auto Reply `ai_auto_reply_run:{run_id}:{attempt_count}:{llm_call_stage}`（三维 identity：run.id 稳定父级 + attempt_count 持久化 invocation identity（claim 原子递增，4 条件全满足）+ llm_call_stage 区分 primary/retry_combined；Preview 保持 chargeable + None 兼容）④ M02 `webhook_event:{event.id}:lead_usage` ⑤ Return Visit Judge `return_visit_run:{run_id}:judge`（9000→9100 跨进程透传 run_id）⑥ Daily Report `daily_report_generation:{generation_id}:summary`（方案 B 独立持久 DailyReportGeneration 实体 + 方案 C billing-report replay only：Stage 5C-2 验真结论 B「1:N Job 是 parent」→ 5C-3 技术设计（DailyReportGeneration billing identity 层，三层身份分离 Job(parent)/Generation(billing)/generation_token(lease)，3 实施约束：创建与 claim 同事务原子 + current_generation_id 确定性引用 + billing truth 只归 M07 无 is_billed）→ 5C-4 实施（migration 0032 + model + claim 改造 + 9000→9100 透传 generation_id + 9100 _report_usage 构造 key + 7 Gate DR-1~DR-7 PASS；full-request response-lost 登记 SEPARATE_REQUEST_RECOVERY_GAP 非 P1 职责，M07 IDEMPOTENCY_CONFLICT 兜底））。**Charge Path Migration Register**（10 条唯一事实源）：6 IDENTITY_VERIFIED + MIGRATED / 1 POLICY_PENDING+IDENTITY_OPEN（Preview CHARGEABLE）/ 3 DESIGN_GAP（M05 ark_v1 固定分析器版本非 per-execution identity / Training training_id 后置 / RAG Embedding chunk 行后置 INSERT）。**关键设计合同**：Business Event ID 必须计费前持久化 + retry/restart 复用；Same Key + Same Stable Inputs → IDEMPOTENT_REPLAY（不重新定价）；Same Key + Different Stable Inputs → IDEMPOTENCY_CONFLICT（不覆盖原 evidence）；PG Concurrent = P1 closure 硬门。COMPUTE-IDEMPOTENCY-001 仍 OPEN（4/10 charge path 仍 None，6/10 已迁移覆盖所有"有稳定执行 identity"的生产路径，剩余 4 个需先有 identity 设计才能迁移），Remediation = PARTIAL_REMEDIATION_VERIFIED。
