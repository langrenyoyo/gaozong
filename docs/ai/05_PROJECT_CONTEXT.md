...
- **2026-08-09 项目治理阶段 3A P1 COMPUTE-IDEMPOTENCY-001 实施进行中（9/11 charge path MIGRATED，检查点 `docs/architecture/remediation/P1_CHECKPOINT_9_OF_11.md`，tag `p1-checkpoint-9-of-11`）**：按 CROSS_MODULE_RISK_REGISTER P1 优先级，先做技术方案设计（`docs/architecture/remediation/P1_COMPUTE_IDEMPOTENCY_TECHNICAL_DESIGN.md`，TECHNICAL_DESIGN_APPROVED）+ M01 身份验真（`docs/architecture/remediation/P1_M01_IDENTITY_VERIFICATION.md`，结论 B：run.id + attempt_count + llm_call_stage 三维 identity），再分 Stage 实施。**M07 Core**（commit cb86c3b/2e81eaf/4109f9d）：record_usage 加 idempotency_key + payload_evidence（stable inputs SHA-256）+ DB migration 0030 + 事务原子性修正（INSERT flush → balance UPDATE → 单次 commit）+ IntegrityError replay/conflict handling + PG_CORE_RELEASE_GATE PASS。**Consumer 迁移**（9/11 MIGRATED）：① M04 `wechat_task:{task.id}:result_usage`（E2E_VERIFIED_FIXED_FOR_M04）② M06 `las_job:{job.id}:archive_usage` ③ M01 Auto Reply `ai_auto_reply_run:{run_id}:{attempt_count}:{llm_call_stage}`（三维 identity）④ M02 `webhook_event:{event.id}:lead_usage` ⑤ Return Visit Judge `return_visit_run:{run_id}:judge` ⑥ Daily Report `daily_report_generation:{generation_id}:summary`（方案 B DailyReportGeneration，migration 0032，7 Gate，PG BLOCKED_BY_SCHEMA_BASELINE_MISMATCH）⑦ Training `knowledge_training_execution:{execution_id}:ask`（方案 B KnowledgeTrainingExecution，migration 0004，TR-5A/5B 账务红线，PG_VERIFIED_MIDPOINT）⑧ RAG Ingest `rag_embedding:{run_id}:{document_id}:{chunk_index}:ingest`（document_id+chunk_index，选项 A durable commit，RI-0/6B PG_VERIFIED_MIDPOINT）⑨ M05 `material_analysis_execution:{execution_id}:ark_analysis`（方案 B AiEditMaterialAnalysisExecution，不激活 dormant Process，C1 红线 Ark 成功→COMPLETED 先于 usage report，migration 0033，7 Gate MA-0~MA-6，PG BLOCKED_BY_SCHEMA_BASELINE_MISMATCH）。**Charge Path Migration Register**（11 条，#10 RAG 拆 #10a/#10b）：9 MIGRATED / 2 OPEN。2 条 OPEN：#7 Preview（CHARGEABLE+POLICY_PENDING+EXECUTION_IDENTITY_DESIGN_GAP，1:N(2) primary+retry_combined，需新建 PreviewExecution）/ #10a RAG Query（EXECUTION_IDENTITY_DESIGN_GAP+CARDINALITY_VERIFIED，1:1+1:2 timeout，需新建 SearchExecution+stage）。**PG Midpoint 结果**：M07 Core PG_VERIFIED / Training 0004 PG_VERIFIED_MIDPOINT / RAG Ingest 事务边界 PG_VERIFIED_MIDPOINT / Daily Report 0032 + M05 0033 BLOCKED_BY_SCHEMA_BASELINE_MISMATCH（auto_wechat 开发库无 alembic_version，create_all 建表，待 DB-BL-2 基线修复）。**PG Closure Gate 三态冻结**：PASS / FAIL / WAIVED_WITH_ACCEPTED_RESIDUAL_RISK（WAIVED≠PASS）。**Reliability Gap**（5 个均 OUT_OF_P1）：DAILY_REPORT/TRAINING/RAG_INGEST_RUN/RAG_INGEST_REQUEST/M05_ANALYSIS_USAGE_REPORT（RUN≠REQUEST 不合并；same execution replay→P1 保护 / full request retry→未保证→P1 不解决）。**关键设计合同**：Business Event ID 必须计费前持久化 + retry/restart 复用；Same Key+Same Inputs→REPLAY / Same Key+Different Inputs→CONFLICT；PG Concurrent=硬门。COMPUTE-IDEMPOTENCY-001 仍 OPEN（2/11 None，9/11 已迁移覆盖所有"有稳定执行 identity 或可前置 identity"的生产路径），Remediation=PARTIAL_REMEDIATION_VERIFIED。
