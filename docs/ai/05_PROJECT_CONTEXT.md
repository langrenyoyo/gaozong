...
- 更新时间：2026-07-18（AI剪辑已按甲方要求进入 `FROZEN_BY_CUSTOMER`：基础 MVP、代码、迁移、数据、测试和 Task 11 测试包保留，但停止 Task 12、干净机复测、重建、分发和生产验证。甲方书面指示只是恢复前提，仍须重新探索、规划和审批）。
...
7. **ORM / 迁移 / 代码写入契约三者必须一致**：新增或修改字段时，`app/models.py` ORM 类型、对应 Alembic `create_table`/`alter_column` 声明类型、service 写入值类型三者必须对齐。SQLite→PG 一次性迁移脚本（如 `scripts/migrate_agents_accounts_core_sqlite_to_postgres.py` 的 `json_fields`/`datetime_fields` 分类）是契约真源。典型反面：ORM `String`/`Text` + service 写 `str`/`json.dumps()`，但迁移建成 `TIMESTAMPTZ`/`JSONB`，PG 拒绝 text→jsonb 隐式转换抛 `ProgrammingError`。`DouyinAuthorizedAccount` 的 `bind_time`/`unbind_time`/`source_created_at`/`raw_body_json` 已于 2026-07-20 收敛为 `DateTime(timezone=True)` + `JSON`，写入用 aware datetime + dict（无时区上游时间按 `Asia/Shanghai` 解释转 UTC，见 `parse_upstream_datetime`）；**仓库代码已修复，生产代码尚未发布验证**，宝塔生产仍需重新部署 + 重测授权回跳确认根因消除。
