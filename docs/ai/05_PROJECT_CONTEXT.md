...
- 商户添加抖音号时，`DY_AUTH_REDIRECT_URL` 必须指向 9000 `/integrations/douyin/live-check/auth-redirect`；`oauth-callback` 仅为观察接口，不写授权账号。前端授权状态只认本次 OAuth `state` 与可信当前商户，历史账号或进程内上次回调不得判定本次成功；仅当本次账号已进入当前商户正式账号列表后才能自动关闭弹窗。
