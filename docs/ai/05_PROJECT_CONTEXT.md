...
- 更新时间：2026-07-25（抖音客服工作台商户隔离、会话已读协议和 webhook 原子幂等 R3-T1 已通过独立测试并快进集成至 master；AI 自动回复 outbox R2-T1 已通过独立测试并快进集成至 master@a245e23，重启恢复测试 R1-T1（R1-R11 全部通过）已通过独立测试并快进集成至 master@a7f924d；NewCar 商户自助改密四态分流、管理员当前浏览器退出与普通退出统一接入并发 401 抑制边界保持有效；AI剪辑继续保持 `FROZEN_BY_CUSTOMER`，恢复仍须重新探索、规划和审批）。
