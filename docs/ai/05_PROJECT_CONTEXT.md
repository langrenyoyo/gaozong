...
- **2026-08-09 项目治理阶段 3A P1 COMPUTE-IDEMPOTENCY-001 实施进行中（8/11 charge path MIGRATED，检查点 `docs/architecture/remediation/P1_CHECKPOINT_8_OF_11.md`）**：按 CROSS_MODULE_RISK_REGISTER P1 优先级，先做技术方案设计（`docs/architecture/remediation/P1_COMPUTE_IDEMPOTENCY_TECHNICAL_DESIGN.md`，TECHNICAL_DESIGN_APPROVED）+ M01 身份验真（`docs/architecture/remediation/P1_M01_IDENTITY_VERIFICATION.md`，结论 B：run.id + attempt_count + llm_call_stage 三维 identity），再分 Stage 实施。**M07 Core**（commit cb86c3b/2e81eaf/4109f9d）：record_usage 加 idempotency_key + payload_evidence（stable inputs SHA-256：capability_key/model/tokens/usage_measurement_method/llm_call_stage，EXCLUDE source/agent_id/conversation_id）+ DB migration 0030（compute_transactions 加 idempotency_key + payload_evidence + UniqueConstraint(merchant_id, idempotency_key)）+ 事务原子性修正（INSERT flush → balance UPDATE → 单次 commit，非分次 commit）+ IntegrityError replay/conflict handling（非 ON CONFLICT）+ None 兼容路径（warning 可追踪）+ PG_CORE_RELEASE_GATE PASS（PostgreSQL 并发：同 idempotency identity → 1 txn / 1 balance mutation / 不同 payload → IDEMPOTENCY_CONFLICT / winner evidence 不被覆盖）。**Consumer 迁移**（8/11 MIGRATED）：① M04 `wechat_task:{task.id}:result_usage`（E2E_VERIFIED_FIXED_FOR_M04）② M06 `las_job:{job.id}:archive_usage` ③ M01 Auto Reply `ai_auto_reply_run:{run_id}:{attempt_count}:{llm_call_stage}`（三维 identity）④ M02 `webhook_event:{event.id}:lead_usage` ⑤ Return Visit Judge `return_visit_run:{run_id}:judge`（9000→9100 跨进程透传）⑥ Daily Report `daily_report_generation:{generation_id}:summary`（方案 B 独立持久 DailyReportGeneration 实体 + 方案 C billing-report replay only，migration 0032，7 Gate DR-1~DR-7，PENDING_PG_VERIFICATION）⑦ Training `knowledge_training_execution:{execution_id}:ask`（方案 B 独立 KnowledgeTrainingExecution 实体，复用 request_id，execution 在 RAG search 前 durable commit，LLM fallback=COMPLETED_FALLBACK 非 failed，migration 0004，9 Gate 含 TR-5A/5B 拆分账务红线，PENDING_PG_VERIFICATION）⑧ RAG Ingest `rag_embedding:{run_id}:{document_id}:{chunk_index}:ingest`（child discriminator = document_id + deterministic chunk_index，chunk_hash 不进 billing key；选项 A `_create_training_run` 后 durable commit；PG 失败 finalize rollback→fresh tx；7 Gate RI-0~RI-6B；不新增表不预 INSERT；PENDING_PG_VERIFICATION 事务边界）。**Charge Path Migration Register**（11 条唯一事实源，原 #10 RAG Embedding 拆为 #10a Query + #10b Ingest）：8 IDENTITY_VERIFIED + MIGRATED / 3 OPEN。3 条 OPEN：#7 Preview（CHARGEABLE + POLICY_PENDING + EXECUTION_IDENTITY_DESIGN_GAP，1:N(2) primary+retry_combined，需新建 PreviewExecution）/ #8 M05（EXECUTION_IDENTITY_DESIGN_GAP，复用 AiEditMaterialProcess.content_analysis stage 行，ark 路径待接入）/ #10a RAG Query（EXECUTION_IDENTITY_DESIGN_GAP + CARDINALITY_VERIFIED，1:1 normal + 1:2 timeout 边界，需新建 SearchExecution + stage）。**证据等级**：runtime 实跑（created/replay/2-charge/None=0）+ CODE_VERIFIED inspect（部分 Gate 非 runtime mock，★ inspect PASS ≠ E2E PASS，PG Closure 前补 runtime 证据）。**PG Closure Gate 三态冻结**：PASS（所有 None=0 + PG gates pass → E2E_VERIFIED_FIXED）/ FAIL（未批准 None path 或 PG 验证失败）/ WAIVED_WITH_ACCEPTED_RESIDUAL_RISK（管理层接受剩余风险，但不得标 E2E_VERIFIED_FIXED，WAIVED≠PASS）。**Reliability Gap 登记**（均 OUT_OF_P1，不阻塞迁移）：DAILY_REPORT/TRAINING/RAG_INGEST_RUN/RAG_INGEST_REQUEST 四个 REQUEST_RECOVERY_GAP（RUN_RECOVERY ≠ REQUEST_RECOVERY 不合并）。**关键设计合同**：Business Event ID 必须计费前持久化 + retry/restart 复用；Same Key + Same Stable Inputs → IDEMPOTENT_REPLAY；Same Key + Different Stable Inputs → IDEMPOTENCY_CONFLICT；PG Concurrent = P1 closure 硬门。COMPUTE-IDEMPOTENCY-001 仍 OPEN（3/11 charge path 仍 None，8/11 已迁移覆盖所有"有稳定执行 identity 或可前置 identity"的生产路径，剩余 3 个需 scope 决策或独立 identity 设计），Remediation = PARTIAL_REMEDIATION_VERIFIED。
