...
- **2026-08-06 抖音私信手机号强制掩码（平台权限，根因性约束）**：抖音开放平台 2026-07-22 通知，2026-08-04 生效——`im.direct_message` 权限保留但私信会话内容中**手机号及固定电话号被掩码**（如 `138****8002`），要解码明文需申请新权限 `im.leads_message_decode`（私信消息解码能力）。**影响**：webhook 收到的是平台已掩码文本，非代码脱敏——空号追问块4 recover 路径（客户重发号码也被掩码，`_PHONE_RE` 不匹配）、线索识别整个链路（拿不到明文）。**解法**：抖音已回复接入新 OpenAPI 文档接口即可（文档已归档 `docs/ai/04_interface_contracts/抖音OpenAPI/抖音私信能力对外OpenApi.docx`，含 `/get_aweme_auth_url`/`content`/`/send_msg`/`/download_resource`/`/list_bind_info`）。P0 止血 `_mask_latest_message_for_llm` 处理平台已掩码文本，分两种语义占位：消息含完整手机号/微信号（经 `extract_contacts_in_text` 命中）→ `[客户已提供完整手机号]`；平台已星号脱敏 `138****8002` 模式 → `[客户已提供联系方式，平台已脱敏]`。是治标，根因解法是对接新接口。
