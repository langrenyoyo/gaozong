...
| 轨道 | 位置 | 说明 |
|---|---|---|
| SQLite 顺序迁移 | `migrations/migrate_sqlite.py` + `migrations/versions/0001~0045.sql` | 开发/过渡库；0030~0045 为 PG 迁移的 SQLite 镜像版本 |
| PostgreSQL Alembic（9000） | `migrations/postgres/auto_wechat/`（版本 0001~0028） | 覆盖主服务运行表；0014 为算力用量计量，0015 为 AI剪辑素材库历史迁移，0016 为 AI 自动回复 outbox 持久化任务字段，0017 为 `douyin_webhook_events` 商户账号复合索引，0018 为 `douyin_account_autoreply_settings.manual_review_risk_flags_json`（风险转人工黑名单），0019 为 `ai_agents` 11 个商家可配置变量字段（固定提示词模板 V2.0），0020 为 `douyin_account_autoreply_settings.allow_release_manual_required`（账号级放行需人工确认回复开关），0021 为回访动态场景配置 + ReturnVisitFollowupTask 表，0022 为 `ai_edit_jobs`/`ai_edit_materials` LAS 字段，0023 为 `compute_markup_ratios` 补 ai_edit 行，0024 为 `compute_markup_ratios` 加消耗模式 consumption_mode/fixed_tokens_per_call，0025 为 AI剪辑结果交付闭环（标题三件套/交付归档状态/视频标签/软删除四件套 + artifact 归档字段），0026 为 `customer_profiles` 顾客档案持久化表（P-0-C 阶段1），0027 为 `customer_profiles` 加 contact_invalid_* 空号追问字段，0028 为 `contact_invalid_followup_tasks` 空号追问任务表；冻结不回退已落地迁移 |
| PostgreSQL Alembic（9100） | `migrations/postgres/xg_douyin_ai_cs/`（0001 空基线 + 0002 RAG metadata 7 表 + 0003 `llm_call_logs.conversation_id` 列 bigint→varchar(255) + 0004 `knowledge_training_executions` Training durable identity + 0005 `rag_search_executions` RAG Query billing identity 实体）| head=0005；本地 dev PG `xg_douyin_ai_cs` 库已于 2026-08-11 经 alembic upgrade 0004→0005；RAG Query 0005 consumer PG verification = PG_RUNTIME_VERIFIED（2026-08-11 独立审批 APPROVED_WITH_CORRECTIONS，BLOCKED_BY_LOCAL_DOCKER_ENVIRONMENT = RESOLVED）|

注意：`wechat_tasks` 是历史遗留——SQLite 主线库由 ORM create_all 建、不在 PG Alembic 0001~0028 中（SQLite 0029 用 `CREATE TABLE IF NOT EXISTS` 壳统一）；PG 由 0003 建。
...
- 一期口径：从“回复建议”收束为 **AI 托管自动回复闭环**；自动回复只走“绑定智能体 + 开关”，“回复建议”作为独立产品形态移除，但 `build_reply_suggestion` 函数保留为预览/自动回复共用 9100 入口（见 8.3 节）。
...
- **2026-08-05 P-0-C 顾客档案持久化阶段1**（`CUSTOMER-PROFILE-PERSIST-1`，commit 9dbd3a0）：把客户已事实确认的内容持久化到数据库，作为 LLM"顾客档案"上下文，解决 AI 跨会话遗忘+重复收集已确认信息的问题。**新增表**（迁移 0026，auto_wechat 库）：`customer_profiles`（gender/preferred_salutation/intent_car/car_year/budget/city/contact_state + confirmed_fields_json 客户明确确认 + inferred_fields_json LLM 推断 + source），唯一约束 merchant_id+account_open_id+customer_open_id 商户隔离。**读写链路**（9100 无状态+9000 持久化）：9000 读取 `build_reply_conversation_context` 读 DB 档案→`merge_profile_with_memory` 合并(DB优先)→注入 customer_memory→known_customer.info 含 salutation/gender；9000 写入 9100 返回 `customer_profile_update`→`dry_run_service` 解析后 `upsert_customer_profile`(SAVEPOINT 隔离+商户归属校验+并发冲突幂等更新，写入失败不阻断主流程)。**LLM 输出 schema**：`_parse_structured_llm_decision` 解析 `customer_profile_update` 字段；`ReplySuggestionResponse`/`V2` 增字段透传；Prompt 输出格式增"顾客档案推断"指令（LLM 每轮输出 gender/preferred_salutation/intent_car/car_year/budget/city/update_reason）。**称呼逻辑**：gender=unknown/male→"老板"，female→"女士"，preferred_salutation 非空用客户要求称呼。**三端隔离**：预览只读(proxy 不写)、训练端不读不写(ask 独立路径)、自动回复可写。**联系方式状态**(contact_state)为只读镜像，写入走 P0.2 contact_state 链路。**未改**：Prompt 回复优化(阶段2)、双消息、调度、发送 gate。回归：P0.2+P0.3+dry_run+LLM 无新增失败。**2026-08-06 三源统一（任务 4.0，commit 96a57da）**：工作台 `_conversation_profile_payload`（`douyin_workbench_conversation_service.py`）与线索列表 `_derive_lead_profile_fields`（`lead_management_service.py`）原只从消息派生画像（`merge_profile_fields`），不读 `customer_profiles` 表——表里有值却展示不出。现两处在消息派生后调 `load_customer_profile`，持久化字段非空覆盖 `intent_car/car_year/budget/city` 四个展示字段（gender/preferred_salutation 不展示），表空时消息派生兜底，读表异常 try/except 降级不阻断。入参：工作台 `merchant_id/first.account_open_id/first.open_id`，线索 `lead.merchant_id/lead.account_open_id/lead.source_id`（=customer_open_id）。三源（工作台/线索列表/LLM）现统一以 `CustomerProfile` 表为单一可信源，LLM 链路 `merge_profile_with_memory` 仍是参考实现（不改）。
...
- **2026-08-06 线索识别增强（任务 2.1 清洗管道 + 2.2 号段白名单 + 缺口一致性修复）**：对照 `docs/ai/01_product_prd/线索识别AI技能规则.md` 增量增强 `app/services/contact_extractor.py`（纯函数，不碰 DB/发送/状态机结构）。**2.1 清洗管道**：`_extract_phone_with_pipeline` 独立式递进清洗（S1 全角转半角 NFKC → S2 剔字母 → S3 剔中文 → S4 全剔非数字 re.UNICODE 覆盖 emoji → S5 中文数字映射+全剔，`CN_DIGIT_MAP` 含大写/谐音幺拐洞吧勾），标准 `_PHONE_RE` 未命中时兜底，只兜手机号不做微信号混淆。**2.2 号段白名单**：`_is_operator_phone`/`_OPERATOR_PREFIXES`（三大运营商+广电号段 frozenset），`1[3-9]xxxxxxxxx` 格式但号段不在白名单 → 降级 `partial_phone`（供 LLM 追问），不进 phones。**缺口修复**：① `analyze_contact_state` step1 `_extract_loose_phone` 未命中后调 `_extract_phone_with_pipeline` 兜底（与 extract 对齐清洗能力）；② `analyze_contact_state` step1 接 `_is_operator_phone`，非白名单降级 PARTIAL（reason_code=`invalid_operator_prefix`）；③ extract 标准匹配失败后走清洗管道兜底。修复后 `extract_contacts_from_text` 与 `analyze_contact_state` 对全角/中文数字/夹字母/区号号码判定一致。**缺口1区号修复**：`_strip_country_code` 正则从锚定行首改为前瞻式 `(?:\+|00)?86(?=[\s\-]*1[3-9])`，允许区号前有中文/字母等非数字前缀（`电话+8613812345678`→`电话13812345678`），前瞻要求 86 后跟合法手机号开头防误伤号码内子串。**已知局限**：兜底号码 `start/end=-1`（原文散落），`mask_contacts_in_text` 用 `value.replace` 脱敏可能替换不到原文片段（升级路径：脱敏改基于区间）。测试：对照规则文档 7.1 全 12 用例 extract vs analyze 一致性全 PASS，完整 contact 回归 300+ passed 无新增失败。
- **2026-08-06 CustomerMemory schema 补字段**：`apps/xg_douyin_ai_cs/schemas.py::CustomerMemory` 加 4 字段 `salutation`/`gender`/`field_sources`/`contact_invalid`（均 `str|dict|None=None`）。**根因**：9000 `merge_profile_with_memory` 注入这 4 字段到 merged dict，但 9000→9100 是 HTTP 调用，9100 用 `ReplySuggestionRequest`（`CustomerMemory` schema）Pydantic 反序列化，schema 无这 4 字段 → 静默丢弃 → `_customer_memory_slots` 的 `getattr` 永远 None → P-0-C 阶段3 field_sources 标注、空号追问块4 contact_invalid 被动兜底、称呼落库三个能力**静默失效**。补字段后全链路通（9000 merge 注入→HTTP JSON→9100 schema 反序列化→`_customer_memory_slots` 提取→`_build_known_customer_context` 注入 known_customer.info→LLM）。
- **2026-08-06 抖音私信手机号强制掩码（平台权限，根因性约束）**：抖音开放平台 2026-07-22 通知，2026-08-04 生效——`im.direct_message` 权限保留但私信会话内容中**手机号及固定电话号被掩码**（如 `138****8002`），要解码明文需申请新权限 `im.leads_message_decode`（私信消息解码能力）。**影响**：webhook 收到的是平台已掩码文本，非代码脱敏——空号追问块4 recover 路径（客户重发号码也被掩码，`_PHONE_RE` 不匹配）、线索识别整个链路（拿不到明文）。**解法（已对接，任务 3.7，commit 358b571）**：`app/services/douyin_resource_download_service.py::decode_msg_content` 复用 `call_douyin_openapi("/decode_msg_content", payload)`，参数与 OpenAPI 文档表26一致（main_account_id/open_id(企业号=to_user_id)/guest_uid(客户=from_user_id)/conversation_id/msg_id）；`app/routers/integrations.py` webhook 收到 `has_encoded=="true"` 且 `im_receive_msg` 时同步调 decode 取明文，替换 `content.text` 供后续 `extract_contacts_from_text` 用明文，解码失败双重 try/except 保留掩码文本不阻断 webhook。msg_id 24h 有效期，webhook 实时事件必在窗口内。OpenAPI 文档归档 `docs/ai/04_interface_contracts/抖音OpenAPI/抖音私信能力对外OpenApi.docx`。**P0 止血兜底仍保留**：`_mask_latest_message_for_llm` 处理平台已掩码文本（decode 失败或非 im_receive_msg 时仍走语义占位），分两种：消息含完整手机号/微信号 → `[客户已提供完整手机号]`；平台已星号脱敏 `138****8002` 模式 → `[客户已提供联系方式，平台已脱敏]`。
- **2026-08-06 customer_profiles JSONB/TEXT 统一（任务 3.2，迁移 0029）**：0026 建表时 `confirmed_fields_json`/`inferred_fields_json` 用 `sa.Text()`，但 ORM 用 `_JSONStringJSONB`（PG 期望 JSONB）→ 生产（ORM create_all 建）是 JSONB、新环境（alembic 建）是 TEXT，结构分裂。**修复**：新增迁移 0029 把两列 `ALTER COLUMN TYPE JSONB`（`postgresql_using="...::text::jsonb"` 处理已存 JSON 字符串转换，幂等——对已是 JSONB 的列无副作用）；0026 修正 `sa.Text()` → `JSONB(none_as_null=True)` 让新环境从 0001 升 head 直接建 JSONB。0027/0028 不受影响（不碰这两列）。闭合 [[jsonb-orm-text-mismatch-recurrent]] 记忆里记录的 customer_profiles 不一致。**生产执行 0029 前**须先跑校验 SQL 确认无非法 JSON：`SELECT id FROM customer_profiles WHERE confirmed_fields_json IS NOT NULL AND confirmed_fields_json::text !~ '^\s*[\{\[]'`。
- **2026-08-06 客服工作台素材后端自动下载（任务 5.0 第一阶段，commit ccd0390）**：`douyin_resource_download_service.download_douyin_resource`（调 `/download_resource` + 存 `DouyinMessageResourceDownload` 表）原是 Manual-only（仅 `douyin_live_check.py` 手动调用），webhook 收到 image/video/emoji 消息不自动下载。现 `app/routers/integrations.py` 新增 `maybe_schedule_resource_download`：webhook 收到 `im_receive_msg` 且 `message_type ∈ {image,video,emoji}` 时 BackgroundTasks 异步调度 `_run_resource_download_task`（SessionLocal + 幂等查重同 `server_message_id` 已有非 failed 记录则跳过 + 从持久化事件取 `merchant_id`）→ 调 `download_douyin_resource`（复用，内部做商户隔离+URL校验+存表）→ 失败记 failed 不阻断 webhook。emoji 按图片格式回调映射为 image。**5.0 第二阶段（前端渲染）待做**：聊天窗口读 `DouyinMessageResourceDownload.download_url` 按 media_type 渲染图片/视频/emoji。
- **2026-08-06 前端主包懒加载优化（任务 3.5）**：`frontend/src/pages/Index.tsx` 原 14 个 feature 页面组件静态 import（DouyinAiCsWorkbenchPage/LasRemixWorkbench/WechatAgent 等）全打到主包，主包 ~1MB（gzip 276kB）。改为 `React.lazy(() => import(...))` + 单个 `<Suspense>` 包裹页面切换链（fallback 用已 import 的 RefreshCwIcon spinner），共享组件（ChatPanel/ContactInfo/ContactList/SideNav）保持静态 import 避免重复打包。主包从 997kB→495kB（-50.4%），gzip 276kB→159kB（-42.2%），各 feature 拆成独立 chunk 按需加载。构建无报错无警告。
- **2026-08-07 线索识别引擎置信度+上下文增强（任务 2.3/2.4/2.5）**：对照规则文档 4.1/0.3/8.1 完善联系方式识别核心机制（纯函数增强 `contact_extractor.py` + `douyin_webhook.py`，不改 DB/发送/状态机结构）。**2.3 置信度字段（commit 417ebf2）**：`ContactExtractResult` 加 `confidence: float=0.0`，按匹配方式分级（标准+白名单 1.0 / 清洗 S0-S4 0.8 / S5 中文数字 0.7 / 非白名单降级 0.4 / 微信关键词 0.95 / 微信全文本 0.85 / 无匹配 0.0）；`_extract_phone_with_pipeline` 改返回 `tuple[str,float]`，extract 取最高 confidence，非白名单 override 0.4；与五态并存不替代，不改 ContactState 状态机。**2.4 LEAD_REQUEST 上下文（commit 0023961）**：`is_lead_request_message` 纯函数检测 AI 出站消息含留资引导关键词（`_LEAD_REQUEST_KEYWORDS`：联系方式/手机号/留个/加我等）；`_has_recent_lead_request` 查客户当前消息紧前的 AI 出站消息（`DouyinPrivateMessageSend` send_source∈ai_auto/return_visit_auto，5分钟窗口，不跨客户/会话/账号/商户，status=sent）；命中时 `context_boost=True`，`_combine_recent_customer_text` 拼接窗口从 5 分钟扩展到 10 分钟；失败降级 False 不影响现有。**2.5 干扰词降权（commit 1df3ef0）**：`_INTERFERENCE_KEYWORDS`（公里/km/迈/万/元/块/价格/年/款/排量，规则文档 8.1）+ `_has_nearby_interference`（前后 6 字符窗口，标准匹配用 start/end 精确定位、管道兜底用 text.find，km 大小写不敏感）；phone 紧邻干扰词降 confidence——context_boost=True 降 0.05、False 降 0.2，不低于 0.0；context_boost=True 时 confidence +0.05 封顶 1.0（规则文档 0.3 第②项，闭合 2.4 TODO）；只降 phone 不降 partial_phone，不改 ContactState 状态机。`extract_contacts_from_text` 加 `context_boost: bool=False` 可选参数，webhook 传 context_boost。线索识别引擎 2.x 系列（2.1 清洗管道 / 2.2 号段白名单 / 2.3 置信度 / 2.4 LEAD_REQUEST / 2.5 干扰词降权）全部完成，核心机制（规则文档 0.3 + 1.x 手机号识别 + 4.1 置信度 + 8.1 干扰词）落地。
- **2026-08-07 客服工作台素材前端渲染（任务 5.0 第二阶段，commit ef60552）**：后端 `_conversation_messages_payload`（`douyin_workbench_conversation_service.py`）批量 join `DouyinMessageResourceDownload`（按 `server_message_id` IN 查询避免 N+1，只带 `resource_status=="success"`，同 msg_id 取 id 最大最新成功记录），消息 payload 加 `download_url`/`resource_media_type`/`resource_status` 三字段，两处调用点（list_conversation_messages + get_conversation_detail）传 db。前端 `DouyinMessageItem` 加 3 字段，`mediaTypeForDownload` 扩展识别 emoji→image，新增 `MessageMedia` 子组件按 media_type 渲染 `<img>`/`<video>`（onError 显示"资源链接已失效"占位不白屏，保留手动下载按钮 fallback），`renderableResource` 判断 download_url+success+image/video。5.0 完整闭环（第一阶段后端自动下载 ccd0390 + 第二阶段前端渲染 ef60552）。
- **2026-08-07 前端手动标记空号 API+UI（任务 1.4，commit ef3319b）**：新增 `POST /admin/contact-invalid/mark`（router `admin_contact_invalid_mark.py`，参考 admin_test_customer_reset 风格），复用 `mark_contact_invalid` service 做状态迁移 VALID→INVALID。**lead_id 强锚定**：`db.get(DouyinLead, lead_id)` + merchant/account/customer 三字段匹配校验（LEAD_MERCHANT_MISMATCH/LEAD_ACCOUNT_MISMATCH/LEAD_CUSTOMER_MISMATCH），不靠会话匹配猜客户。**块3触发**：`mark_contact_invalid` 返回 new_version 非 None（状态迁移）时调 `create_followup_task`（trigger_source="admin_manual_mark"，幂等唯一约束防重）。权限 `auto_wechat:admin:autoreply`，reason 白名单（空号/打不通/号码错误/停机/其他），失败 try/except + rollback 不阻断。前端 `LeadsManagement.tsx` LeadDetail 加"标记为空号"按钮（amber 配色），prompt 选原因 + window.confirm 二次确认 + toast 反馈 + refreshData。替代已删 Task2A，作为空号追问链路的触发源。
- **2026-08-07 销售微信回写空号识别断点修复（任务 1.6，commit 45e34a5）**：`app/services/wechat_ui_reply_service.py::agent_write_back_reply`（Local Agent 回写链路）原只做关键词匹配判 replied，**不调空号识别**——与 `reply_checker.record_manual_reply`（手动录入）不一致。现补齐：`find_effective_reply` 后插入空号识别（与 record_manual_reply 对齐）：合并 analyze_msgs 文本 → `analyze_contact_validity` → invalid 调 `mark_contact_invalid`（source="wechat_reply"）+ `create_followup_task` 触发块3（trigger_source="wechat_reply"，比 record_manual_reply 多块3触发）；valid 调 `recover_contact_valid`。lead_id 强锚定（lead.source_id=customer_open_id）。失败 try/except 不阻断回写主流程。空号追问链路 4 触发源（前端手动/微信回写/状态/解码）全部闭环。
- **2026-08-07 项目治理阶段 1「项目现实地图」闭环（1A.1-1A.5）**：按 `docs/ai/项目治理目标.md` 的"Index First, Move Later"原则，只探索+文档化不改业务行为，建立 `docs/architecture/` 导航层。**1A.1 SYSTEM_MAP**（commit 6ced0ba/c26ec22）：系统全景——运行组件（进程/服务 React 5173+9000+9100+19000 / 数据基础设施 PostgreSQL 方案A一实例两库 / 外部运行依赖 Milvus+NewCar+GMP+LAS+TOS）+ 7 业务模块（M01-M07）+ 公共能力（平台 6 类+领域共享 1 类联系方式提取）+ 外部系统 6 个 + 数据域（auto_wechat 54 表 8 域+xg_douyin_ai_cs 7 表）+ 模块依赖 + 12 项已知 Legacy 标记。探索发现：MILVUS_URI 非 HOST/PORT、NEWCAR_AUTH_BASE_URL、素材库与剪辑共用目录、算力兼容入口。**1A.2 CODE_INDEX**（commit fc39c8f/e5781a3）：`CODE_INDEX.yaml`（机器可读，schema_version=2）+ `CODE_INDEX.md`（生成）+ `generate_code_index.py`（生成器）。7 模块 M01-M07 结构化索引，每模块 frontend/backend/workers/external_services/tables/config/tests/dependencies（带 module/type/direction/reason 结构）/lifecycle_candidates/status。**1A.3 RUNTIME_ENTRYPOINTS**（commit 06557cb/246048c）：9 类入口（Frontend Routes/FastAPI Routers/Webhook/Scheduler/Worker/Startup/CLI/Docker/Callback）+ env gate 盲区汇总（16 个 env 开关）+ Worker 拆分（Persistent/Durable vs In-process BackgroundTask）+ CLI execution_class 分类 + Local Agent API vs Callback 区分 + scheduler_runtime_model=in_process_thread + lifecycle≠runtime_state 区分 + CONTACT_INVALID_FOLLOWUP_ENABLED 标 CONFIG_BYPASS + @app.on_event 标 TECH_DEBT。**1A.4 DEPENDENCY_MATRIX**（commit c9b1ddb/827fd00）：7×7 矩阵 + 11 Canonical Edge（E1-E11，事实源）+ 5 类型（R runtime/D data/X contract-payload/S shared-impl/E event=0）+ mechanism 列 + 公共底座依赖 + 外部系统依赖 + 5 条治理观察（M01 依赖中心/M02 数据 Owner/M05-M06 解耦候选/M07 纯 Provider/M04→M02 回写）。**1A.5 LEGACY_REGISTER**（commit c67a52d/d924e01）：15 项 Legacy 定性（LEGACY×5/COMPAT×5/DEAD_CANDIDATE×2/ACTIVE×2/UNKNOWN×1），每项 9 字段（能力/代码位置/状态/新方案/启用条件/生产默认/运行证据/是否允许删除/删除前置）。5 种生命周期正式定义 + TECH_DEBT 作 quality_flags 不作 lifecycle + Lifecycle≠Deletion Eligibility + UNKNOWN 禁止无证据修改。**治理循环建立**：地图→验真→冻结→清废→解耦→回归→更新地图。下一步进入阶段 2 逐模块验真（M03 智能体→M01 客服→M02 线索→M04 微信助手→M05 素材→M06 剪辑→M07 算力）。
- **2026-08-08 项目治理阶段 2「模块验真」2A+2B 完成（7/7 模块）**：按治理目标逐模块验真，每模块 8 步（业务能力→入口→运行链→数据资产→Legacy→E2E→基线→解耦）。**PHASE 2A Current Reality**：7 模块全部完成代码事实验真（M03→M01→M02→M04→M05→M06→M07），每模块 6 份文档（README/CURRENT_FLOW/DATA_MODEL/RUNTIME_DEPENDENCIES/ACCEPTANCE/ISSUES）+ file:line 证据。**PHASE 2B Controlled E2E + Baseline Candidate**：7 模块全部 Docker E2E 完成（部分需 R1/R2 补 fixture），全部形成 `BASELINE_CANDIDATE`（非 `MODULE_BASELINE_APPROVED`）。**关键跨模块发现**：① COMPUTE-IDEMPOTENCY-001（M07 record_usage 无幂等键，E2E_VERIFIED FINANCIAL_INTEGRITY，M04 E2E_VERIFIED_IMPACTED consumer）② M04-001 DUPLICATE_EXECUTION_RISK（无 claim/lease，两 Agent 拿同一 Task E2E 证明）③ M05-004 临时 URL 持久化（M06 LAS 消费，dependency impact VERIFIED）④ M05-005 引用检查类型不匹配（active job 素材可误删 E2E 证明）⑤ ISSUE-M02-007 CLOSED（M04 反向证明 feedback contract MATCH，root cause: E2E fixture 缺 DB 上下文）。**治理模式确立**：环境阻塞→BASELINE_CANDIDATE 固化→开放 Gate 保持→推进下一模块→环境恢复后补证据。**Consumer 证据四级**：E2E_VERIFIED_IMPACTED / CODE_VERIFIED_EXPOSED / CALL_SITE_IDENTIFIED / NOT_VERIFIED。**CROSS_MODULE_RISK_REGISTER 冻结**（commit 35dd4f4）：4 HIGH 生产安全风险（P1 Compute Idempotency / P2 M04 Claim / P3a M05 Reference / P3b M05 URL）+ 5 STRUCTURAL 架构风险（S1 Lead Identity / S2 Agent Config / S3-S4 DATA_COUPLING / S5 Shared Implementation）分队列。**PHASE 2C External Gate OPEN**：26 Gate Records（Staging 16 / Windows 6 / External 6），共享 STAGING_E2E_READINESS + WINDOWS_E2E_READINESS。下一步进入 PHASE 3A Production Safety Stabilization（P1 Compute Idempotency 技术方案设计，先设计审批不直接改码）。
- **2026-08-10 项目治理阶段 3A P1 COMPUTE-IDEMPOTENCY-001 ★ Consumer Migration 11/11 COMPLETE（里程碑检查点 `docs/architecture/remediation/P1_CHECKPOINT_11_OF_11_CONSUMER_MIGRATION_COMPLETE.md`，tag `p1-checkpoint-11-of-11-consumer-migration-complete`）**：Consumer 层工作完成，进入 Technical Closure。**CONSUMER_MIGRATION=COMPLETE / TECHNICAL_CLOSURE=PENDING**（★ ≠ E2E_VERIFIED_FIXED）。**11 条 charge path 全部 MIGRATED**：① M04 `wechat_task:{task.id}:result_usage` ② M06 `las_job:{job.id}:archive_usage` ③ M01 Auto Reply `ai_auto_reply_run:{run_id}:{attempt_count}:{llm_call_stage}` ④ M02 `webhook_event:{event.id}:lead_usage` ⑤ Return Visit `return_visit_run:{run_id}:judge` ⑥ Daily Report `daily_report_generation:{generation_id}:summary`（0032 BLOCKED_BY_SCHEMA_BASELINE_MISMATCH）⑦ Training `knowledge_training_execution:{execution_id}:ask`（0004 PG_VERIFIED_MIDPOINT）⑧ RAG Ingest `rag_embedding:{run_id}:{document_id}:{chunk_index}:ingest`（事务边界 PG_VERIFIED_MIDPOINT）⑨ M05 `material_analysis_execution:{execution_id}:ark_analysis`（0033 BLOCKED_BY_SCHEMA_BASELINE_MISMATCH）⑩ M01 Preview `ai_preview_execution:{preview_execution_id}:{llm_call_stage}`（0034 BLOCKED_BY_SCHEMA_BASELINE_MISMATCH）⑪ RAG Query `rag_search_execution:{search_execution_id}:{embedding_stage}`（0005 PENDING_PG_VERIFICATION/BLOCKED_BY_LOCAL_DOCKER_ENVIRONMENT）。**Technical Closure Blockers（4 项）**：A. auto_wechat schema baseline（DB-BL-2，解锁 0032/0033/0034）/ B. RAG Query 0005 PG（Docker 恢复后独立补）/ C. Global Active None Audit（重新全局搜索）/ D. Final PG Concurrent Closure Gate。**PG Closure Gate 三态冻结**：PASS / FAIL / WAIVED_WITH_ACCEPTED_RESIDUAL_RISK（WAIVED≠PASS）。**Reliability Gap**（7 个均 OUT_OF_P1）：DAILY_REPORT/TRAINING/RAG_INGEST_RUN/RAG_INGEST_REQUEST/M05_ANALYSIS_USAGE_REPORT/PREVIEW_REQUEST/RAG_QUERY_REQUEST（same execution replay→P1 保护 / full request retry→未保证→P1 不解决）。**核心设计原则**：Business Event ID 必须计费前 durable commit + Same Key+Same Inputs→REPLAY / Different→CONFLICT + Execution.status≠Billing truth（M07 sole ledger）。**治理有效性证明**：已拦截 5 类误去重（M01 run_id 缺 attempt / M05 ark_v1 误去重 re-analysis / DailyReportJob.id 误去重 regenerate / RAG chunk_hash 撞相同内容 / RAG Query _search_sqlite 函数名误标 fallback）。COMPUTE-IDEMPOTENCY-001 仍 OPEN，Remediation=CONSUMER_MIGRATION_COMPLETE。**2026-08-10 PostgreSQL application-role prerequisite 闭环（APPROVED）**：`APPLICATION_ROLE_PERMISSION_GAP = RESOLVED — LOCAL DEVELOPMENT`，`APPLICATION_ROLE_PREREQUISITE = VERIFIED`。canonical dev PG（`auto-wechat-postgres-dev`@5432，db=auto_wechat，head=0034）已完成 ownership correction（`ALTER DATABASE auto_wechat OWNER TO postgres`）+ 最小权限 GRANT（DML 60 业务表 + SELECT alembic_version + 60 sequences USAGE/SELECT）+ alembic_version SELECT-only 硬收敛（C3 GRANT→REVOKE 顺序）+ 未来对象 ADP（`FOR ROLE postgres IN SCHEMA public`，无 TRUNCATE）；`auto_wechat` 应用角色非 superuser / database CREATE=false / public schema CREATE=false / DDL+TRUNCATE 负向 DENIED / `/ready` 以 `auto_wechat` PASS（HTTP 200 / postgresql / 0034）。实施报告 `docs/architecture/remediation/P1_PG_APPLICATION_ROLE_PERMISSION_IMPLEMENTATION_REPORT.md`，审批 `..._IMPLEMENTATION_APPROVAL.md`（APPROVED）。**0032/0033/0034 = UNBLOCKED_FOR_PG_VERIFICATION**（不等于 PG_VERIFIED，consumer runtime evidence 尚未执行，须由独立 consumer PG verification 窗口以 `auto_wechat` 应用角色实施）。**staging/prod = CONFIG_VERIFIED / RUNTIME_UNKNOWN**（不得写 permissions fully resolved everywhere）。**LOCAL_PG_BOOTSTRAP_DATABASE_OWNER_DRIFT_GAP = RESOLVED**（2026-08-11 独立实施审批 APPROVED_WITH_CORRECTIONS，`P1_PG_BOOTSTRAP_OWNER_DRIFT_IMPLEMENTATION_APPROVAL.md`）：`docker/postgres/init/001_create_databases.sql:20` 收敛为 `CREATE DATABASE auto_wechat OWNER postgres`（Gap①，fresh bootstrap 即正确 owner，dev-only）；新增 `scripts/pg/bootstrap_app_role_permissions.sql`（Gap②，幂等 GRANT/ADP/REVOKE + fail-closed guard）+ `scripts/pg/bootstrap_local_dev_pg.py`（C2 post-Alembic caller：alembic upgrade head→permission bootstrap，alembic 失败 STOP）。独立审批窗口用独立隔离 PG `au-pg-approval-iso`@25434 复现 FB-0~FB-12 全 PASS（含 FB-11 幂等 run#1/#2 等价、FB-12 零手工干预），canonical DB no-drift（owner=postgres/head=0034/tables=61 不变）；`FRESH_BOOTSTRAP_APPLICATION_ROLE_GRANT_GAP = RESOLVED`，`FRESH_BOOTSTRAP_PRINCIPAL_REPRODUCIBILITY = VERIFIED`。当前运行库 COMPLIANT 不变，staging/prod 仍 NO WRITE / RUNTIME_UNKNOWN。P1 仍 OPEN / TECHNICAL_CLOSURE=PENDING。RB-10 NOT AUTHORIZED。
