...
- 更新时间：2026-07-16（Phase 12 Task 0-10 + Task 10-FIX1~FIX6 已完成，检查点 A/B/C 均 PASS，状态 `DONE_WITH_CONCERNS`，唯一 concern `baota_ai_edit_production_not_verified`；Task 11 单入口 EXE 已用测试 API `https://merchant.xiaogaoai.cn/api` + 前端 `https://merchant.xiaogaoai.cn/` + 商户 `m_nc_2bba00063cc13016` 重建，闭合公网前端 CORS/PNA、干净机 token 商户绑定和重复 App 初始化问题，状态 `BUILT_FOR_CUSTOMER_TEST`，SHA-256 `20C7918B49B46C1173875888C33B673E750C9F5F42D3263CD468CE2CBC8C821C`；新包待干净虚拟机复测，Phase 13 完成前不得启动宝塔生产验证）。
...
1. AI剪辑纳入一期；Phase 12 Task 0-10 + Task 10-FIX1~FIX6 已完成，检查点 A/B/C 均 PASS，状态 `DONE_WITH_CONCERNS`，唯一 concern `baota_ai_edit_production_not_verified`。本地唯一流程为前端→19000→9000，禁止双创建；Task 11 单入口 EXE 当前 `BUILT_FOR_CUSTOMER_TEST`（API 基址含 `/api`，公网前端 CORS/PNA、严格回环鉴权、单 token 商户绑定、Worker 凭据隔离与单 App 启动均已验证，SHA-256 `20C7918B49B46C1173875888C33B673E750C9F5F42D3263CD468CE2CBC8C821C`），新包待干净虚拟机复测；正式安装包和宝塔生产验证均未启动。
...
| Phase 12 AI剪辑本地 MVP | Task 0-10 + Task 10-FIX1~FIX6 已完成，检查点 A/B/C 均 PASS，状态 `DONE_WITH_CONCERNS`，唯一 concern `baota_ai_edit_production_not_verified`；Task 11 单入口 EXE 当前 `BUILT_FOR_CUSTOMER_TEST`（API=`https://merchant.xiaogaoai.cn/api`，CArchive 配置、CORS/PNA 预检、内部 Local Agent、鉴权三态、9 路由、单 App 启动与端口释放通过，SHA-256 `20C7918B49B46C1173875888C33B673E750C9F5F42D3263CD468CE2CBC8C821C`），新包待干净虚拟机复测；正式安装包未启动，宝塔生产验证必须等 Phase 13 完成后统一执行 |
