...
- 更新时间：2026-07-16（Phase 12 Task 0-10 + Task 10-FIX1~FIX6 已完成，检查点 A/B/C 均 PASS，状态 `DONE_WITH_CONCERNS`，唯一 concern `baota_ai_edit_production_not_verified`；Task 11 单入口 EXE 已用测试 API `https://merchant.xiaogaoai.cn/api` + 前端 `https://merchant.xiaogaoai.cn/` + 商户 `m_nc_2bba00063cc13016` 重建，闭合公网前端 CORS/PNA、干净机 token 商户绑定、重复 App 初始化和离线 EasyOCR 打包问题，状态 `BUILT_FOR_CUSTOMER_TEST`，SHA-256 `C1F5930B40D8F6A0E15D872A6EC8983847908B6EF20FEB4A92BB4C433058CF07`；新包待干净虚拟机复测，Phase 13 完成前不得启动宝塔生产验证）。
