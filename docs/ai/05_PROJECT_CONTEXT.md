...
P4-3 已完成（同步后自动分配联动：auto_assign=true 对新建线索自动调用 assign_service.auto_assign_next()）

下一阶段：P4-4 定时同步（后台定时拉取 + 端到端验证）
...
| auto_wechat | 中间业务执行层 | `E:\work\project\auto_wechat` | P3 完成，P4 开发中 |
| React UI | 客户运营后台前端原型 | `E:\work\project\react` | 纯前端 Mock 原型 |
...
- 分配异常不回滚整批同步
- dry_run=true 时 reason 标记"dry_run 未执行自动分配"

P4-4 待实现（定时同步/端到端验证）：

- 后台定时拉取 douyinAPI 线索
- 端到端验证（douyinAPI → auto_wechat → 分配 → 检测）
...
### P5：React UI 接入真实 API

目标：

逐步替换 React UI 中的 Mock 数据，接入 auto_wechat 和 douyinAPI 真实接口。

接入方向：

- auto_wechat：线索管理、销售管理、检测记录
- douyinAPI：线索详情、会话消息
...
未来：React 页面逐步替换 Mock 数据，接入 auto_wechat 和 douyinAPI 真实接口。
