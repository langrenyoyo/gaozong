...
- 更新时间：2026-07-16（Phase 12 Task 0-10 + Task 10-FIX1~FIX6 已完成，检查点 A/B/C 均 PASS，状态 `DONE_WITH_CONCERNS`，唯一 concern `baota_ai_edit_production_not_verified`；Task 11 单入口 EXE 已用真实测试 API `https://merchant.xiaogaoai.cn` + 前端 `https://merchant.xiaogaoai.cn/` + 商户 `m_nc_2bba00063cc13016` 重新构建，烘焙配置经 CArchive 提取确认不含占位地址，状态 `BUILT_FOR_CUSTOMER_TEST`，SHA-256 `173AF6C255E59DFC89517CC2C0E330FBE3FCB3D6B490FB1790133B8DF40578E0`；Phase 13 完成前不得启动宝塔生产验证）。
...
1. AI剪辑纳入一期；Phase 12 Task 0-10 + Task 10-FIX1~FIX6 已完成，检查点 A/B/C 均 PASS，状态 `DONE_WITH_CONCERNS`，唯一 concern `baota_ai_edit_production_not_verified`。本地唯一流程为前端→19000→9000，禁止双创建；Task 11 单入口 EXE 已用真实测试 API / 前端地址 / 商户 ID 重新构建并标 `BUILT_FOR_CUSTOMER_TEST`（SHA-256 `173AF6C255E59DFC89517CC2C0E330FBE3FCB3D6B490FB1790133B8DF40578E0`），token 不入包、19000 回环鉴权保持开启、Worker 凭据隔离生效；正式安装包和宝塔生产验证均未启动。
...
- 小高算力（Phase 10）：本地模拟闭环和三方复审已完成；字符计量、六能力上浮、三快照、前端与权限闭环已落地，支付仍为 mock。总状态 `DONE_WITH_CONCERNS`，遗留项为 `baota_production_compute_not_verified`。
...
| Phase 12 AI剪辑本地 MVP | Task 0-10 + Task 10-FIX1~FIX6 已完成，检查点 A/B/C 均 PASS，状态 `DONE_WITH_CONCERNS`，唯一 concern `baota_ai_edit_production_not_verified`；Task 11 单入口 EXE 当前 `BUILT_FOR_CUSTOMER_TEST`（真实 URL 构建 + 内部 Local Agent 启动 / health / 鉴权三态通过，烘焙配置经 CArchive 确认不含占位地址，SHA-256 `173AF6C255E59DFC89517CC2C0E330FBE3FCB3D6B490FB1790133B8DF40578E0`）；正式安装包未启动，宝塔生产验证必须等 Phase 13 完成后统一执行 |
