...
- **抖音客服会话已读协议（DY-CS-CONVERSATION-READ-PROTOCOL-1）候选已实现，待独立测试确认**（2026-07-23）：
  - `mark-read` 请求新增必填正整数 `last_seen_event_id`；缺失或非法返回 422，且无状态写入。
  - 服务端在可信商户、有效绑定账号和指定会话范围内验证该事件；不可见或不属于该会话的事件统一返回防枚举 404，且无状态写入。
  - 水位精确推进到 `last_seen_event_id`，不得取服务器当前最新事件替代；已读水位使用 `(created_at, event_id)` 单调前进。
  - 并发保护落在数据库条件更新（`UPDATE ... WHERE (created_at, event_id) < new`）和唯一约束竞争处理（`IntegrityError` 重试）上，禁止仅做应用层先读后比。
  - 重复或旧请求返回 200 和当前水位，不得回退。
  - 未读计算继续只统计 `im_receive_msg`，并按 `(created_at, event_id)` 判断。
  - 前端仅在当前详情请求成功并完成 React 状态提交后提交已读，从本次响应消息取最大有效 `raw_event_id`；`raw_event_id` 缺失或非法时不得猜测、不得提交已读；缓存消息不得作为本次详情成功的证据。
  - 删除点击会话和详情请求开始前的本地清零；本地清零与服务端提交绑定同一个事件水位；提交失败保留重试能力。
