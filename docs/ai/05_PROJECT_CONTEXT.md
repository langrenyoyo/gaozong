...
- 更新时间：2026-07-27（抖音客服工作台商户隔离、会话已读协议和 webhook 原子幂等 R3-T1 已通过独立测试并快进集成至 master；AI 自动回复 outbox R2-T1 已通过独立测试并快进集成至 master@a245e23，重启恢复测试 R1-T1（R1-R11 全部通过）已通过独立测试并快进集成至 master@a7f924d，PostgreSQL/MVCC 恢复测试 R1-T1（P1-P9、C1-C4 全部 PASS）已通过独立测试并快进集成至 master@df8644d828680a75ff955db59c546d4ba1caa729，9000 PostgreSQL JSONB/ORM 一致性首批返修 R1-T1（J1-J16、B1-B8 全部 PASS）已通过独立测试并快进集成至 master@020ab730bae8ac2c570ce4e0e185f203b62b08e4；NewCar 商户自助改密四态分流、管理员当前浏览器退出与普通退出统一接入并发 401 抑制边界保持有效；AI剪辑继续保持 `FROZEN_BY_CUSTOMER`，恢复仍须重新探索、规划和审批）。
...
| PostgreSQL Alembic（9000） | `migrations/postgres/auto_wechat/`（版本 0001~0016） | 覆盖主服务运行表；0014 为算力用量计量，0015 为 AI剪辑素材库历史迁移，0016 为 AI 自动回复 outbox 持久化任务字段；冻结不回退已落地迁移 |
