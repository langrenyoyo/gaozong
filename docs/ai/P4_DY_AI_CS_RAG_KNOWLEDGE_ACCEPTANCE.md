...
| 知识分类管理页面 | 已完成最小版 | 前端“知识分类”页面支持分类列表和创建 merchant 分类，不做编辑/删除/禁用。 |
| 正式知识库页面 | 已完成最小版 | 前端“知识库”页面支持创建知识文档和手动训练当前分类，不做文档列表、详情、编辑、删除、搜索。 |
| 9000 分类主表 | 已完成 | `knowledge_categories` 主表已落地，`GET /knowledge-categories` 返回 base + 当前商户 active 分类，`POST /knowledge-categories` 支持创建 merchant 分类。 |
...
4. 前端已经具备 Agent 分类多选、知识分类最小管理页面、知识库文档创建与训练最小页面。
5. 自动发送安全边界未放开。
6. Phase 5-E-F 运行态 E2E 已确认：`p5_acceptance_test` 分类、知识文档、训练、Agent 分类绑定、reply-suggestion 分类召回和 `auto_send=false` 主链路通过。
...
7. 生产环境未执行数据库迁移和备份验收前，不得把新表能力视为生产可用。

## 8. 剩余事项

优先级从高到低：

1. 生产环境执行并验收 9000 / 9100 相关数据库迁移。
2. 补充 `p5_blocked_test` 负向分类边界运行态验收。
3. 收敛 `9000` / `9100` conversation_id 数字型契约差异。
4. 将 `searchRag()` 从内部调试能力收口到可信代理或移出产品入口。
5. 后续再评估文档列表、详情、编辑、删除和训练记录页面。
