> **归档声明（2026-07-14 基线重构）**
>
> 本文件是 `docs/ai/05_PROJECT_CONTEXT.md` 在 2026-07-14 文档基线重构（P3-AI-DOCS-BASELINE-REFACTOR-1）前的完整冻结快照，仅用于历史追溯。
>
> - 本文件**不是当前项目事实**，内部存在已知冲突（auto_send 门禁、douyinAPI 定位、前端路径、SQLite/PG 状态、webhook 验签等），且滞后于 P3-Z4/Z5、9100 PG 迁移、Phase 8/9、一键过审取消等后续进展。
> - 当前有效上下文见 `docs/ai/05_PROJECT_CONTEXT.md`（重构后的当前事实文档）。
> - 禁止在本文件中继续维护"当前状态"；禁止把本文件内容当作当前约束引用。
> - 本快照同时覆盖了同期从 `CLAUDE.md` / `AGENTS.md` 移除的历史完成记录（P0-API-1 ~ P0-DEV-E1、P7/P8/P0-3/P0-4 阶段详情等）的等价内容。
...
9100 xg_douyin_ai_cs
  - 负责抖音 AI 客服、RAG、统一知识库 metadata、训练反馈自动入库、LLM 回复决策。

8788 car-porject-main 知识训练入口
  - 作为 AI 抖音客服训练页面和知识训练入口，经 9000 可信代理访问统一知识库能力。

19000 Local Agent
  - 运行在客户 Windows 本机，只负责本机微信 UI 自动化和只读检测；真实发送必须经安全 gate。
...
1. 一期已放开旧的自动发送硬门禁；抖音私信、AI 自动回复和微信派单真实发送仍必须通过后端 gate 与运行保护。
2. 不得绕过违禁词、人工接管、限频、失败回写、幂等、紧急停止。
3. 前端不得把传入的 tenant_id / merchant_id / douyin_account_id 当可信上下文。
...
### 0.0c 2026-07-10 Phase 0-B 入口上下文同步
...
AI小高线索、小高AI微信助手、AI剪辑、一键过审是需要具备独立运行能力的子功能服务，必须预留独立启动、独立部署、独立配置、独立健康检查、独立日志、独立异常处理能力。

AI剪辑由 `auto_edit` 先独立完成，后续源码迁入 `auto_wechat` 仓库；一键过审复制改造 `douyinAPI` 现有实现，运行时不依赖 `douyinAPI`。
...
4. 一键过审不混入小高AI微信助手线索派单链路，按一期独立模块边界接入。
5. 不把 LLM 写入第一版需求。
6. 不把截图保存写入第一版需求。
7. 第一版截图不保存、不入库。
...
关键结论：第一版产品化测试验收必须以生产强制 Webhook 验签、原始 body 签名一致性、私信纯文本联系方式提取、有效线索生成、invalid 展示与导出、状态流转、销售分配、超时重分配、Local Agent 发送 / 检测互斥、失败回写、日志脱敏、数据迁移兼容、前端接口和导出为主线。该记录描述 P1 单次闭环时期的历史验收口径；一期真实发送改造必须另按违禁词、人工接管、限频、失败回写、幂等、紧急停止 gate 验收。
...
7. 生产模板 `.env.production.example` 明确要求 `DOUYIN_WEBHOOK_AUTH_REQUIRED=true` 和 `DY_SECRET_KEY`。
...
Docker 本地开发环境负责启动 `9000 + 9100 + frontend`（`docker-compose.dev.yml` 是独立完整开发编排，非 production `docker-compose.yml` 的 override）：
...
# P3-D5 douyin_leads runtime shadow read 与 observability 当前状态

任务：`P3-D5-DB-9000-LEADS-RUNTIME-SHADOW-READ-AND-OBSERVABILITY-1`

当前已在 P3-D4 默认关闭脚手架基础上，扩展 `douyin_leads` 运行态 read-only shadow，并补充 shadow 观测：
...
4. `douyin_leads` shadow 必须携带 `merchant_id`；缺失时跳过，不做无隔离查询。
5. mismatch、异常、timeout 只记录结构化日志和内存指标，不影响用户响应。
6. 结构化日志只记录摘要与 `pii_redacted=True`，不记录完整手机号、微信号、客户名或数据库密码。
7. 未切换 `DATABASE_URL`，未改业务接口默认数据库，未默认开启 PG pilot。
...
# P3-D11 leads/tasks worker/pool sizing benchmark 当前状态

任务：`P3-D11-DB-9000-LEADS-TASKS-UVICORN-MULTI-WORKER-POOL-SIZING-1`

当前已在 P3-D10 真实 Uvicorn/HTTP benchmark scaffold 基础上新增 worker/pool sizing benchmark：
...
1. 默认仍不启用 PG pilot。
2. 默认仍不切换 `DATABASE_URL`。
3. PG pilot/read shadow 未开启时，新配置不生效。
4. `sampled_out` / `concurrency_limited` 不影响 SQLite 主响应，不连接 PostgreSQL，不视为 error。
5. metrics 新增 sampled_out、concurrency_limited 和 inflight 观测。

worker benchmark 覆盖：

1. Uvicorn worker 数矩阵。
2. PG `pool_size` / `max_overflow` 矩阵。
3. shadow max concurrency / sample rate 矩阵。
4. `estimated_pg_connections = workers * (pool_size + max_overflow)`。
5. HTTP throughput / p50 / p95 / p99 / error_rate。
6. shadow metrics 与 engine manager snapshot。

边界确认：

1. P3-D11 仍只使用本地/dev synthetic。
2. P3-D11 不连接宝塔 production。
3. P3-D11 不读取 production SQLite。
4. P3-D11 不执行 production apply。
5. P3-D11 不切换默认 `DATABASE_URL`。
6. P3-D11 不默认开启 PG pilot。
7. P3-D11 不启用 PG write。
8. P3-D11 不接入 webhook write、pending task、task result write、`notify_sales` / `detect_reply` 写链路。
9. P3-D11 不触发 LLM、抖音发送、微信发送、私信发送或自动回复 gate。
10. P3-D11 benchmark 不代表 production QPS600 达标。

下一步建议：`P3-D12` 做 shadow sampling / max concurrency 策略调优，或 `P3-E1` 进入智能体 / 抖音账号绑定 schema batch。

# P3-D12 leads/tasks shadow sampling / concurrency tuning 当前状态

任务：`P3-D12-DB-9000-LEADS-TASKS-SHADOW-SAMPLING-CONCURRENCY-TUNING-1`

当前已在 P3-D11 worker/pool sizing benchmark 基础上完成本地/dev synthetic sample rate 与 shadow max concurrency 调优：
...
5. `tuning_summary.recommended_gray_config`。
...
2. P3-D12 未连接宝塔 production。
3. P3-D12 未读取 production SQLite。
4. P3-D12 未执行 production apply。
5. P3-D12 未切换默认 `DATABASE_URL`。
6. P3-D12 未默认开启 PG pilot。
7. P3-D12 未启用 PG write。
8. P3-D12 未接入 webhook write、pending task、task result write、`notify_sales` / `detect_reply` 写链路。
9. P3-D12 未触发 LLM、抖音发送、微信发送、私信发送或自动回复 gate。
10. P3-D12 benchmark 仍不代表 production QPS600 达标。

下一步建议：`P3-D13` 做 runtime shadow gray config preset 与环境变量文档，默认关闭；或 `P3-E1` 进入智能体 / 抖音账号绑定 schema batch。

# P3-D13 leads/tasks shadow gray preset 与 Runbook 当前状态

任务：`P3-D13-DB-9000-LEADS-TASKS-SHADOW-GRAY-PRESET-AND-RUNBOOK-1`

当前已基于 P3-D12 本地/dev synthetic tuning 结果，新增 leads/tasks PostgreSQL read-only shadow 灰度预设、启停 Runbook 和上线前准入检查：

```text
docs/ai/03_data_and_migration/LEADS_TASKS_SHADOW_GRAY_PRESET_RUNBOOK.md
```

`.env.example` 已补充默认关闭的注释示例；该变更不改变运行时默认行为。
...
1. dev recommended：可用于本地/dev synthetic 验证，显式开启 PG pilot/read shadow，PG write 必须 false。
2. staging cautious：需要人工审批，建议从 `shadow_sample_rate=0.05`、`shadow_max_concurrency=5` 起步，并设置观察窗口和回滚负责人。
3. production current：`not approved / not executed`，必须保持 `LEADS_TASKS_PG_PILOT_ENABLED=false`、`LEADS_TASKS_PG_READ_SHADOW_ENABLED=false`、`LEADS_TASKS_PG_WRITE_ENABLED=false`。
...
2. 当前仍不默认开启 PG pilot。
3. 当前仍不启用 PG write。
4. 当前未连接宝塔 production。
5. 当前未读取 production SQLite。
6. 当前未执行 production apply。
7. 当前未接 webhook write、pending task、task result write、`notify_sales` / `detect_reply` 写链路。
8. 当前未触发 LLM、抖音发送、微信发送、私信发送或自动回复 gate。
...
# P3-F2 compute core data migration dry-run 与 dev apply 当前状态

任务：`P3-F2-DB-9000-POSTGRESQL-COMPUTE-DATA-MIGRATION-DRY-RUN-AND-DEV-APPLY-1`

当前已为 P3-F1 的 compute 两表新增 SQLite -> PostgreSQL 数据迁移 dry-run / apply 工具链：
...
1. `compute_accounts` 按 `merchant_id`。
2. `compute_transactions` 按 `id`，使用 SQLite id 映射到 PostgreSQL id。
3. 当前业务没有 `transaction_id` / `idempotency_key`，本轮不提前新增。

数据校验：

1. `balance_tokens`、`delta_tokens`、`balance_after_tokens` 必须保持整数。
2. `delta_tokens = 0` 进入异常行，不静默 apply。
3. 非整数 token 字段进入异常行。
4. datetime 解析失败进入异常行。
5. dry-run 输出 per-table summary 与 total summary。
...
8. 本轮未修改支付、扣费、充值、套餐发放或流水生成逻辑。
9. 本轮未触发 LLM、抖音发送、微信发送、私信发送或自动回复 gate。

下一步建议：

1. `P3-F3`：compute SQLite vs PostgreSQL contrast。
2. `P3-G0`：P1 表分级决策，不能直接全量迁移或切换默认数据库。

# P3-Z0 9000 PostgreSQL cutover gap audit 当前状态

任务：`P3-Z0-DB-9000-POSTGRESQL-CUTOVER-GAP-AUDIT-1`

当前已完成 9000 从 SQLite 切换到 PostgreSQL 的最短可行缺口审计：
...
1. 当前仍不能直接切换 production `DATABASE_URL` 到 PostgreSQL。
2. 当前应停止继续对 leads/tasks 做单表深挖，转入 staging cutover gap closure。
3. 直接切库第一硬阻塞是 `app/database.py` 同步主 engine 仍拒绝 PostgreSQL backend，且 `app/main.py` 启动阶段仍使用 `Base.metadata.create_all(bind=engine)`。
4. PostgreSQL 已覆盖 11 张表：`knowledge_categories`、leads/tasks 四表、agents/accounts 四表、compute 两表。
5. 仍缺失多张 runtime 会访问的 PG schema，包括 `external_merchant_bindings`、`reply_checks`、`check_configs`、`lead_notifications`、`lead_followup_records`、`feedback_records`、自动回复/工作台相关表和 `compute_packages`。
6. `python scripts/check_sqlite_specific_usage.py` 已执行，结果为 `errors=0, warnings=80`；warning 均在允许清单内。
...
1. P3-Z0 只做只读代码审计和文档。
2. P3-Z0 未新增 migration，未迁移数据，未执行 Alembic。
3. P3-Z0 未连接宝塔 production，未读取 production SQLite。
4. P3-Z0 未切换 `DATABASE_URL`，未开启 PG pilot，未启用 PG write。
5. P3-Z0 未触发 LLM、抖音发送、微信发送、私信发送或自动回复 gate。

# P3-Z1 9000 PostgreSQL runtime gap schema batch 当前状态

任务：`P3-Z1-DB-9000-POSTGRESQL-RUNTIME-CUTOVER-GAP-SCHEMA-1`

当前已新增 PostgreSQL Alembic revision：
...
本批补齐 P3-Z0 审计中缺失的 19 张 runtime 表：
...
1. 9000 PostgreSQL schema 覆盖面已从 11 张表扩展到 30 张 runtime 表。
2. 本轮只补 schema，不迁移 SQLite 数据。
3. 本轮未执行 production apply，未连接宝塔 production，未读取 production SQLite。
4. 本轮未切换默认 `DATABASE_URL`，未开启 PG pilot，未启用 PG write。
5. `app/database.py` PostgreSQL 主 engine 启动路径和 `Base.metadata.create_all` 风险仍需后续受控处理。

下一步建议：

1. `P3-Z2`：补齐 cutover 必需表数据迁移脚本。
2. `P3-Z3`：staging PG `DATABASE_URL` 启动 smoke。
3. 仍不得跳过 staging smoke 和 production 审批直接切换默认数据库。

# P3-Z2 9000 PostgreSQL cutover 一次性迁移脚本当前状态
...
1. 脚本覆盖 30 张 9000 runtime 表。
2. 默认 dry-run，不写 PostgreSQL。
3. apply 必须显式 `--apply --yes`。
4. apply 拒绝 `APP_ENV=production`，拒绝隐式 `DATABASE_URL`，只允许 dev/staging host 和 `auto_wechat` database。
5. 本轮未执行宝塔 staging apply，未连接宝塔 production，未读取 production SQLite。
6. 本轮未切换默认 `DATABASE_URL`，未开启 PG pilot，未启用 PG write。

下一步建议：

1. `P3-Z3`：9000 使用 PostgreSQL `DATABASE_URL` 的 staging 启动 smoke。
2. P3-Z3 需要先处理受控 PostgreSQL 主 engine 启动路径和 production 禁用自动 `Base.metadata.create_all` 策略。

# P3-Z3 9000 PostgreSQL DATABASE_URL startup smoke scaffold 当前状态

任务：`P3-Z3-DB-9000-POSTGRESQL-DATABASE-URL-STARTUP-SMOKE-1`

当前已补齐 9000 主服务使用 PostgreSQL `DATABASE_URL` 创建 app 的最小 runtime 缺口：
...
1. `app/database.py` 不再直接拒绝 PostgreSQL backend。
2. 9000 主同步 SQLAlchemy runtime 使用 `psycopg`；规划中的 `postgresql+asyncpg://` 会派生为同步 `postgresql+psycopg://`。
3. SQLite 默认路径不变，仍自动建表。
4. PostgreSQL 下不执行 `Base.metadata.create_all`，必须先通过 Alembic 初始化 schema。
5. startup smoke 只验证 app import / create_app，不进入 lifespan，不启动 scheduler、热键或桌面浮层。

边界确认：

1. 本轮未连接宝塔 production，未读取 production SQLite。
2. 本轮未执行 production apply，未迁移真实数据。
3. 本轮未切换 production `DATABASE_URL`。
4. 本轮未默认开启 PG pilot，未启用 PG write。
5. 本轮未触发 LLM、抖音发送、微信发送、私信发送或自动回复 gate。

下一步建议：

1. 在宝塔 staging 安装更新后的 requirements，确认 `psycopg[binary]` 可用。
2. staging PostgreSQL 先 Alembic 到 head，再执行 startup smoke。
3. 继续 `P3-Z4` 核心页面 / 核心接口 smoke，不能直接进入 production cutover。
