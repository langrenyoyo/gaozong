...
| 目标阶段 | Phase 12 AI剪辑迁入准备 |
| 审计方式 | 只读源码审计、调用链追踪、离线测试、目标项目承接能力对照 |
| 文档状态 | 正式迁入专项计划的输入，不代表源码已经迁入或功能已经上线 |

## 2. 阶段目标与边界

### 2.1 本阶段目标

本阶段只解决一个问题：明确 `auto_edit` 哪些内容值得迁、迁到哪里、迁入前必须修什么，以及 `auto_wechat` 需要提供什么运行边界。

现有一期总控计划将 Phase 12 定义为“AI剪辑迁入预留”，并明确正式源码迁入需要另开专项计划：

- `docs/superpowers/plans/2026-07-10-xiaogao-ai-phase1-master-plan.md:594-624`
- `docs/superpowers/plans/2026-07-10-xiaogao-ai-phase1-master-plan.md:802`
...
3. 给出迁入分层、目标架构、数据契约和验收门槛。
4. 将结论归档到 `docs/ai/13_ai_edit/` 并更新文档索引。

### 2.3 本阶段禁止事项

1. 不复制 `auto_edit/src`、`scripts` 或 `runs` 到本仓库。
2. 不新增 AI剪辑路由、Worker、数据库迁移、Docker 镜像或前端接口。
3. 不调用真实 LLM，不下载真实视觉模型，不生成客户视频。
...
```text
React
  -> 9000：鉴权、auto_wechat:ai_edit、可信商户上下文、任务与产物控制面
  -> 独立 apps/ai_edit Worker：原子领取、下载输入、执行剪辑、上传产物、回写状态
  -> 受控文件存储或对象存储：只使用内部 storage_key
  -> 9000：按商户校验后提供预览和下载
```

9000 API 进程不能同步执行 ASR、模型推理或 FFmpeg 长任务。
...
处理要求：字幕坐标按目标画布计算；独立 Worker 镜像安装和预检确定的中文字体，不依赖宿主机字体名称碰运气。
...
`merchant_id` 从 9000 的可信鉴权上下文和数据库关系取得，不能接受前端自报。Worker 再由 `artifact_id` 解析内部 `storage_key`，前端不接触底层路径。
...
4. PostgreSQL 是素材 metadata 真源；不共享 BrollStudio SQLite 文件。
5. 底层素材允许内容去重，但商户资产、标签、审核和任务必须隔离。
6. BrollStudio 原片、增稳片和分析报告分别注册为受控 artifact，不保存外部绝对路径。
...
9000 控制面
  ...
  - 可信 merchant_id
  - 任务和 artifact metadata
  - 上传/预览/下载授权
  - 取消、重试和审计

apps/ai_edit Worker
  - 原子领取任务
  - storage_key 下载到任务级临时目录
  - ASR/视觉/规划/渲染/字幕/BGM
  - 模型进程级缓存
  - 产物上传和完整性回写
  - 心跳、取消和清理
...
1. 保留 `apps/ai_edit` 独立边界。
2. 只提供健康、配置状态和不造假的任务壳。
3. 未配置服务时明确返回 `not_configured`。
4. 不调用外部仓库 CLI。
...
1. 新开正式迁入专项计划，不把它混入 Phase 12 预留壳。
2. 冻结任务、素材候选和 artifact 契约。
3. 建立独立 Worker、原子领取、取消和受控存储。
