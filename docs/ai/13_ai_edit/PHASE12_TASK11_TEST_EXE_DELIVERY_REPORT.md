...
| 大小 | 593,379,652 字节（约 566 MB） |
| SHA-256 | `13A05B75EC3AF7E247EA86FDA26AA88AB2A81C9E4E23380EF4233B42C4BB6681` |
...
- `OPTIONS /agent/ai-edit/materials/{material_id}`（`Access-Control-Request-Method: DELETE`）→ **200**，`Access-Control-Allow-Methods` 为 `GET, POST, DELETE, OPTIONS`；浏览器删除请求不再被预检 400 阻断。
...
### 3.6 相邻回归
```
Local Agent runtime 定向回归：7 passed
AI剪辑路由 + 微信自动化冻结基线：105 passed
前端生产构建：PASS（1926 modules）
```
覆盖：DELETE 私网预检、AI剪辑本地路由、`poll-and-execute`、`poll-and-detect`、诊断安全序列化；均无新增失败。
...
| `app/local_agent_main.py` | CORS 允许方法只有 GET/POST/OPTIONS，素材 DELETE 在进入路由前被浏览器预检 400 阻断 | 保留精确 origin 与 Private Network Access，并把 DELETE 加入允许方法；打包后真实预检返回 200 |
