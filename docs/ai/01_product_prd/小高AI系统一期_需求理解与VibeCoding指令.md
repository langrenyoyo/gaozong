...
- **全局级**，所有商户共享。
- 词库结构：**违禁词**（`safe_word` 为兼容可选字段，G1-DELTA 冻结后不参与替换、不再必填）。
- **过滤机制（G1-DELTA 冻结方案，2026-08-18 甲方确认）**：AI 自动回复 → 命中违禁词 → 最多重生成 1 次（命中词注入第二次请求）→ 仍命中转人工（`manual_required=true`/`auto_send=false`），禁止自动发送；抖音人工私信原文发送；微信派单/通知/反馈完全豁免；抖音自动回访话术发送前检测命中阻断；回访模板命中 → 400 拒绝保存。**不再执行"替换为安全词后自动发送"**。
- 每词一行存储（便于按词查询/统计命中）。
