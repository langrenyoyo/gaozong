...
本阶段不是生产迁移，而是提前建立准生产级迁移规范。
DB-MIG 只负责方案设计。
P2-A 只负责迁移脚本骨架与副本 dry-run（已完成 ✅）。
P2-C 才允许对当前开发测试主线库执行正式迁移（先迁库）。
P2-B 才允许 models.py 字段补齐（后补模型，避免模型字段领先于数据库结构）。
```
...
DB-MIG 方案已输出，P2-A 迁移骨架与副本验证已完成，P2-A-END 验收口径已修正。下一步是等待用户确认后进入 **P2-C：开发测试主线库正式迁移**（先迁库），完成后再进入 **P2-B：models.py 字段补齐**（后补模型）。
...
用户确认是否进入 P2-C
  ↓
P2-C：对当前开发测试主线库 data/auto_wechat.db 执行正式迁移（先迁库）
  ↓
P2-B：确认 P2-C 后，才允许 models.py 字段补齐（后补模型）
  ↓
P3~P12 按阶段边界继续推进
```

> **顺序说明（2026-06-15 调整）**：P2-C 先于 P2-B。原因：若先改 `models.py` 而主线库未迁移，SQLAlchemy 模型会认为新字段已存在但实际表无此列，运行接口 / 查询时缺列报错。先迁移主线库落出新列，再补 models.py，确保数据库结构与模型同步。

进入 P2-C 前必须重新复述 P2-C 的阶段目标、允许修改范围、禁止事项、验收标准，并等待用户确认。
