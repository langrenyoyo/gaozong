...
## 52. P3-F2 compute 数据迁移 dry-run 与 dev apply smoke

任务：`P3-F2-DB-9000-POSTGRESQL-COMPUTE-DATA-MIGRATION-DRY-RUN-AND-DEV-APPLY-1`

P3-F2 已为 P3-F1 创建的两张 compute PostgreSQL 表补充 SQLite -> PostgreSQL 数据迁移脚本、dry-run 能力、静态测试和本地/dev synthetic apply smoke：
...
1. `compute_accounts`：按 `merchant_id` upsert，对齐 `uk_compute_accounts_merchant`。
2. `compute_transactions`：按 `id` upsert，明确使用 SQLite id 映射到 PostgreSQL id；当前业务没有 `transaction_id` / `idempotency_key`，本轮不凭空新增。
3. 脚本不执行 `delete` / `truncate` / `drop`；dev smoke 只按 synthetic merchant/id 范围清理测试数据。

字段与校验口径：

1. `balance_tokens`、`delta_tokens`、`balance_after_tokens` 保持整数语义，不使用 Float。
2. `delta_tokens = 0` 会进入异常行，避免违反 `ck_compute_transactions_delta_nonzero`。
3. 非整数 token 字段会进入异常行，不静默转换为 Float。
4. datetime 解析失败会进入异常行。
5. SQLite 有、PostgreSQL 无的字段记录为 `ignored_fields`；PostgreSQL 可默认或可空字段记录为 `defaulted_fields`。
6. `mapping_preview` 会脱敏 `remark`、`agent_id`、`conversation_id` 等上下文字段。
...
dev synthetic apply smoke 预期：

```text
第一次 dry-run: total_insert=4, total_update=0, total_skip=0, total_errors=0, DRY_RUN_PASS
apply: inserted=4, updated=0, skipped=0, errors=0, APPLY_PASS
第二次 dry-run: total_insert=0, total_update=4, total_skip=0, total_errors=0, DRY_RUN_PASS
PostgreSQL 行数: compute_accounts >= 2, compute_transactions >= 2
SMOKE_PASS: compute core data migration dev apply ready
```
...
9. 本轮未触发 LLM、抖音发送、微信发送、私信发送、自动回复 gate。

当前切库结论不变：P3-F2 只证明 compute 两表迁移脚本在本地/dev synthetic 数据上具备 dry-run、受控 apply 与幂等闭环，不等于宝塔真实数据迁移完成，也不等于可以切换默认数据库。

后续建议：

1. `P3-F3`：compute SQLite vs PostgreSQL contrast。
2. `P3-G0`：P1 表分级决策，继续按表批次推进，不直接全量迁移。

## 53. P3-Z0 PostgreSQL cutover gap audit

任务：`P3-Z0-DB-9000-POSTGRESQL-CUTOVER-GAP-AUDIT-1`

P3-Z0 已停止继续对单表做深挖，转为审计 9000 从 SQLite 切换到 PostgreSQL 的最短可行缺口：
...
1. 现在仍不能直接切换 production `DATABASE_URL` 到 PostgreSQL。
2. 直接切库的第一硬阻塞是 `app/database.py` 的同步主 engine 当前仍在 PostgreSQL backend 下主动拒绝启用，`app/main.py` 又在启动阶段执行 `Base.metadata.create_all(bind=engine)`。
3. PostgreSQL Alembic 当前覆盖 11 张表：`knowledge_categories`、leads/tasks 四表、agents/accounts 四表、compute 两表。
4. ORM/runtime 仍有多张表缺失 PostgreSQL schema，包括 `external_merchant_bindings`、`reply_checks`、`check_configs`、`lead_notifications`、`lead_followup_records`、`feedback_records`、自动回复/工作台相关表和 `compute_packages`。
5. `python scripts/check_sqlite_specific_usage.py` 结果为 `errors=0, warnings=80`；warning 均来自允许清单，主要是 9100 RAG SQLite 兼容层、SQLite migration 和测试。
...
1. P3-Z0 只做只读审计和文档。
2. P3-Z0 未新增 migration。
3. P3-Z0 未迁移数据。
4. P3-Z0 未执行 Alembic。
5. P3-Z0 未连接宝塔 production，未读取 production SQLite。
6. P3-Z0 未切换 `DATABASE_URL`，未开启 PG pilot，未启用 PG write。

## 54. P3-Z1 runtime cutover gap schema batch

任务：`P3-Z1-DB-9000-POSTGRESQL-RUNTIME-CUTOVER-GAP-SCHEMA-1`

P3-Z1 已为 Z0 审计出的 9000 runtime 缺失表新增 PostgreSQL Alembic schema batch：
...
1. 主键使用 PostgreSQL `BigInteger` 自增。
2. 时间字段使用带时区 `DateTime(timezone=True)`。
3. JSON 类字段使用 `JSONB`。
4. Boolean 字段使用 PostgreSQL Boolean，不继续依赖 SQLite 0/1 隐式语义。
5. 只补已有 ORM/runtime 会访问的表和关键索引 / 约束，不新增业务状态机，不 seed 数据。
...
1. 本轮只补 PostgreSQL schema。
2. 本轮不迁移 SQLite 数据。
3. 本轮不执行 production apply。
4. 本轮不连接宝塔 production，不读取 production SQLite。
5. 本轮不切换默认 `DATABASE_URL`。
6. 本轮不默认开启 PG pilot，不启用 PG write。
7. 本轮不触发 LLM、抖音发送、微信发送、私信发送或自动回复 gate。

切库结论：

1. 9000 PostgreSQL schema 覆盖面已从 11 张表扩展到 30 张 runtime 表。
2. 当前仍不能直接切换 production `DATABASE_URL`。
3. `app/database.py` 同步主 engine PostgreSQL 启动路径和 `Base.metadata.create_all` 风险仍需后续受控处理。
4. 下一步是 `P3-Z2` 补齐 cutover 必需表数据迁移脚本，或拆分 runtime PG staging 启动 smoke 任务。

## 55. P3-Z2 9000 cutover 一次性数据迁移脚本

任务：`P3-Z2-DB-9000-POSTGRESQL-CUTOVER-DATA-MIGRATION-1`

P3-Z2 已新增 9000 SQLite -> PostgreSQL cutover 统一迁移脚本和测试：
...
2. `sales_staff`
3. `external_merchant_bindings`
4. `check_configs`
5. `ai_agents`
6. `douyin_authorized_accounts`
7. `douyin_leads`
8. `douyin_webhook_events`
9. `reply_checks`
10. `lead_notifications`
11. `lead_followup_records`
12. `feedback_records`
13. `wechat_tasks`
14. `douyin_oauth_states`
15. `douyin_account_agent_bindings`
16. `agent_knowledge_categories`
17. `douyin_account_autoreply_settings`
18. `conversation_autopilot_states`
19. `douyin_conversation_read_states`
20. `douyin_private_message_sends`
21. `ai_reply_decision_logs`
22. `ai_auto_reply_runs`
23. `douyin_message_resource_downloads`
24. `douyin_image_uploads`
25. `autoreply_rollout_configs`
26. `autoreply_whitelist_entries`
27. `autoreply_admin_audit_logs`
28. `compute_accounts`
29. `compute_transactions`
30. `compute_packages`

脚本安全门：

1. 默认 dry-run，不写 PostgreSQL。
2. `--apply` 必须显式 `--yes`。
3. `APP_ENV=production` 拒绝 apply。
4. apply 不允许隐式使用 `DATABASE_URL`，必须使用 `--postgres-url` 或 `SMOKE_DATABASE_URL`。
5. apply 只允许 dev/staging host，目标 database 必须为 `auto_wechat`。
6. 脚本不执行 drop / truncate / delete；按 `id` 保留 SQLite 主键并做通用 upsert。

边界确认：

1. 本轮新增的是迁移工具链，不执行宝塔 staging apply。
2. 本轮未连接宝塔 production，未读取 production SQLite。
3. 本轮未切换默认 `DATABASE_URL`。
4. 本轮未开启 PG pilot，未启用 PG write。
5. 下一步是 `P3-Z3`：9000 使用 PostgreSQL `DATABASE_URL` 的 staging 启动 smoke。

## 56. P3-Z3 9000 PostgreSQL DATABASE_URL startup smoke scaffold

任务：`P3-Z3-DB-9000-POSTGRESQL-DATABASE-URL-STARTUP-SMOKE-1`

P3-Z3 已补齐 9000 主 runtime 使用 PostgreSQL `DATABASE_URL` 创建 app 的最小缺口：
...
1. `create_database_engine()` 不再拒绝 PostgreSQL backend。
2. 主同步 SQLAlchemy runtime 使用 `psycopg`，并可从规划中的 `postgresql+asyncpg://` 派生同步 URL。
3. PostgreSQL runtime 使用 `DB_POOL_SIZE` / `DB_MAX_OVERFLOW` / `DB_POOL_TIMEOUT` / `DB_POOL_RECYCLE` / `DB_STATEMENT_TIMEOUT_MS`。
4. `app.main.ensure_runtime_schema()` 保持 SQLite 自动建表兼容路径，但 PostgreSQL 下跳过 `Base.metadata.create_all`，要求先跑 Alembic。
5. 新增 startup smoke 脚本，只验证 app import / create_app，不进入 lifespan，不启动 scheduler、热键或桌面浮层。

边界确认：

1. 本轮未连接宝塔 production，未读取 production SQLite。
2. 本轮未执行 migration apply，未迁移真实数据。
...
- 已通过独立测试并快进集成至 `master@df8644d828680a75ff955db59c546d4ba1caa729`（2026-07-27）。
- 版本：`0016`，down_revision=`0015_ai_edit_material_library`（R1-REPAIR-1 修正：原错误缩写 `"0015"` 导致迁移图断裂 `KeyError: '0015'`，修正后指向真实前驱 `0015_ai_edit_material_library`；迁移图唯一 head=0016，`ScriptDirectory` 图解析测试断言）。
- 范围：`ai_auto_reply_runs` 新增 `lease_owner`/`lease_expires_at`/`attempt_count`/`next_attempt_at`/`last_failure_stage`；新增调度/租约索引（`idx_ai_auto_reply_runs_status_next_attempt`、`idx_ai_auto_reply_runs_lease`）。
...
- `_GateResultsJSON` 跨方言合同（R1-REPAIR-2）：`AiAutoReplyRun.gate_results_json` 改为 `JSON(none_as_null=True).with_variant(JSONB(none_as_null=True), "postgresql")`，SQLite 编译 JSON、PostgreSQL 编译 JSONB、`none_as_null=True`、Python `None` 写为 SQL `NULL`；静态合同测试断言跨方言编译与 `ScriptDirectory` 图解析。不修 `ReturnVisitRun.gate_results_json`（其 PG 迁移本就是 Text，一致）。
