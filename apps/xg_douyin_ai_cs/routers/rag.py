...
@router.post("/train", response_model=RagTrainResponse, dependencies=[Depends(require_internal_service_token)])
def train(payload: RagTrainRequest) -> RagTrainResponse:
    data = repository.train_scope(payload)
    return RagTrainResponse(**data)


@router.post("/search", response_model=RagSearchResponse, dependencies=[Depends(require_internal_service_token)])
