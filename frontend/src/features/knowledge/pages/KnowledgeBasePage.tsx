...
function categoryTypeLabel(category?: KnowledgeCategory | null): string {
  ...
  if (category.is_base || category.scope_type === "system") return "基础知识 / 系统内置";
  if (category.scope_type === "merchant") return "商户分类";
  return category.scope_type || "未知分类";
}
      ...
      <header className="shrink-0 border-b border-[#e4e8f0] bg-white px-5 py-4">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-[15px] font-bold text-[#1a1f2e]">知识库</h1>
              <p className="mt-1 text-xs text-[#8b95a6]">
                当前第一版仅支持创建知识文档和触发训练，暂不支持文档列表、编辑和删除。
              </p>
            </div>
          </div>
          <button
          ...
          <div className="mb-4 grid grid-cols-2 gap-4 max-[860px]:grid-cols-1">
            <section className="rounded-xl border border-[#e4e8f0] bg-white p-4 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
              <div className="mb-3 flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-[#1a1f2e]">抖音企业号</h2>
              </div>
              <select
                ...
                {authorizedAccounts.length === 0 ? (
                  ...
                  authorizedAccounts.map((account) => (
                    <option key={account.account_open_id} value={account.account_open_id}>
                      {displayAccountName(account)} / {account.account_open_id} / {account.authorization_status}
                    </option>
                  ))
                )}
              </select>
              <p className="mt-2 text-[11px] text-[#8b95a6]">
                {selectedAccount
                  ? `当前选择：${displayAccountName(selectedAccount)}，授权状态 ${selectedAccount.authorization_status || selectedAccount.status}`
                  : "请选择已授权的抖音企业号。"}
              </p>
            </section>
            ...
            <section className="rounded-xl border border-[#e4e8f0] bg-white p-4 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
              <div className="mb-3 flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-[#1a1f2e]">知识分类</h2>
              </div>
              <select
                ...
                {availableCategories.length === 0 ? (
                  ...
                  availableCategories.map((category) => (
                    <option key={category.category_key} value={category.category_key}>
                      {category.name || category.category_key} / {category.category_key} / {categoryTypeLabel(category)}
                    </option>
                  ))
                )}
              </select>
              <p className="mt-2 text-[11px] text-[#8b95a6]">
                {selectedCategory ? `当前选择：${categoryTypeLabel(selectedCategory)}。base 分类允许写入。` : "请选择可写入的知识分类。"}
              </p>
            </section>
          </div>
          ...
          <section className="rounded-xl border border-[#e4e8f0] bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
            <div className="border-b border-[#e4e8f0] px-5 py-4">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-bold text-[#1a1f2e]">创建知识文档</h2>
              </div>
              <p className="mt-1 text-xs text-[#8b95a6]">
                创建只写入 knowledge_documents；训练完成后，知识 chunks / embedding 才会生成或刷新。
              </p>
            </div>

            <div className="space-y-4 px-5 py-5">
              <label className="grid gap-1.5 text-xs">
                <span className="font-semibold text-[#475569]">知识标题</span>
                <input
                  value={draft.title}
              ...
              <label className="grid gap-1.5 text-xs">
                <span className="font-semibold text-[#475569]">知识内容</span>
                <textarea
                  value={draft.content}
              ...
              <div className="grid grid-cols-2 gap-3 max-[760px]:grid-cols-1">
                <label className="grid gap-1.5 text-xs">
                  <span className="font-semibold text-[#475569]">品牌 brand（可选）</span>
                  <input
                    value={draft.brand}
                ...
                <label className="grid gap-1.5 text-xs">
                  <span className="font-semibold text-[#475569]">车型 vehicle_name（可选）</span>
                  <input
            ...
            <div className="flex items-center justify-between gap-3 border-t border-[#e4e8f0] px-5 py-4 max-[760px]:flex-col max-[760px]:items-stretch">
              <p className="text-xs text-[#8b95a6]">创建成功后仍需点击“训练当前分类”，AI 回复建议才会使用新知识。</p>
              <button
                type="submit"
        ...
        <aside className="min-h-0 overflow-y-auto border-l border-[#e4e8f0] bg-white p-5 max-[1180px]:border-l-0 max-[1180px]:border-t">
          <section className="rounded-xl border border-[#e4e8f0] bg-[#f8fafc] p-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-bold text-[#1a1f2e]">训练当前分类</h2>
            </div>
            <p className="mt-2 text-xs leading-5 text-[#64748b]">
              训练会基于当前企业号与分类刷新知识 chunks / embedding。创建文档不会自动触发训练。
            </p>
          ...
          <section className="mt-4 rounded-xl border border-[#e4e8f0] bg-white p-4">
            <h2 className="text-sm font-bold text-[#1a1f2e]">最近一次创建</h2>
            {lastDocument ? (
              <dl className="mt-3 space-y-2 text-xs">
                <div className="flex justify-between gap-3">
                  <dt className="text-[#8b95a6]">document_id</dt>
                  <dd className="font-semibold text-[#1a1f2e]">{lastDocument.document_id ?? "-"}</dd>
                </div>
                <div className="flex justify-between gap-3">
                  <dt className="text-[#8b95a6]">title</dt>
                  <dd className="min-w-0 truncate font-semibold text-[#1a1f2e]">{lastDocument.title || "-"}</dd>
                </div>
                <div className="flex justify-between gap-3">
                  <dt className="text-[#8b95a6]">category_key</dt>
                  <dd className="font-mono text-[#334155]">{lastDocument.category_key || "-"}</dd>
                </div>
                <div className="flex justify-between gap-3">
                  <dt className="text-[#8b95a6]">status</dt>
                  <dd className="font-semibold text-[#0f766e]">{lastDocument.status || "created"}</dd>
                </div>
              </dl>
            ) : (
              <p className="mt-3 text-xs leading-5 text-[#8b95a6]">尚未创建知识文档。</p>
            )}
          </section>

          <section className="mt-4 rounded-xl border border-[#e4e8f0] bg-white p-4">
            <h2 className="text-sm font-bold text-[#1a1f2e]">最近一次训练</h2>
            {lastTraining ? (
              <dl className="mt-3 space-y-2 text-xs">
                <div className="flex justify-between gap-3">
                  <dt className="text-[#8b95a6]">training_run_id</dt>
                  <dd className="font-semibold text-[#1a1f2e]">{lastTraining.training_run_id ?? "-"}</dd>
                </div>
                <div className="flex justify-between gap-3">
                  <dt className="text-[#8b95a6]">document_count</dt>
                  <dd className="font-semibold text-[#1a1f2e]">{lastTraining.document_count ?? "-"}</dd>
                </div>
                <div className="flex justify-between gap-3">
                  <dt className="text-[#8b95a6]">chunk_count</dt>
                  <dd className="font-semibold text-[#1a1f2e]">{lastTraining.chunk_count ?? "-"}</dd>
                </div>
                <div className="flex justify-between gap-3">
                  <dt className="text-[#8b95a6]">status</dt>
              ...
              <p className="mt-3 text-xs leading-5 text-[#8b95a6]">尚未触发知识训练。</p>
            )}
          </section>

          <section className="mt-4 rounded-xl border border-[#dfe5ee] bg-[#f8fafc] p-4 text-xs leading-5 text-[#64748b]">
            <div>当前企业号：{selectedAccount ? displayAccountName(selectedAccount) : "未选择"}</div>
            <div className="mt-1">account_open_id：{shortId(selectedAccountOpenId)}</div>
            <div className="mt-1">当前分类：{selectedCategory ? `${selectedCategory.name} / ${selectedCategory.category_key}` : "未选择"}</div>
          </section>
        </aside>
      </div>
    </section>
