  ...
  const saveAutoreplySettings = useCallback(async () => {
    ...
    try {
      ...
      const payload: DouyinAutoReplySettingUpdateRequest = {
        ...
        allow_release_manual_required: autoreplySetting.allow_release_manual_required ?? false,
                ...
                <section className="space-y-2">
                  ...
                  <div className="flex items-center justify-between rounded-md border border-slate-200 px-3 py-2">
                    ...
                    <button
                      type="button"
                      role="switch"
                      aria-checked={autoreplySetting.allow_release_manual_required === true}
                      onClick={() =>
                        setAutoreplySetting((current) =>
                          current
                            ? { ...current, allow_release_manual_required: !current.allow_release_manual_required }
                            : current,
                        )
                      }
                      className={`relative h-5 w-10 rounded-full transition-colors ${autoreplySetting.allow_release_manual_required ? "bg-emerald-500" : "bg-slate-300"}`}
                    >
