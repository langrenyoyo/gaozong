...
assertIncludes(app, "const token = getExternalToken();", "logout 先读取当前 NewCar token");
assertIncludes(app, 'apiClient.post("/auth/logout"', "logout 先调用 9000 /auth/logout");
assertIncludes(app, "Authorization: `Bearer ${token}`", "logout 调用透传 Bearer token");
assertIncludes(app, "正在退出登录，请稍候…", "logout 提示文案不应乱码");
